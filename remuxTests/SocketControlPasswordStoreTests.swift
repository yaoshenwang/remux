...
final class SocketControlPasswordStoreTests: XCTestCase {
    ...
    func testSaveLoadAndClearRoundTripUsesFileStorage() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-socket-password-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDefaultPasswordFileURLUsesRemuxAppSupportPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-socket-password-tests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        XCTAssertEqual(
            resolved?.path,
            tempDir.appendingPathComponent("remux", isDirectory: true)
    ...
    func testLegacyKeychainMigrationCopiesPasswordDeletesLegacyAndRunsOnlyOnce() throws {
        ...
        let fileURL = tempDir.appendingPathComponent("socket-password.txt", isDirectory: false)
        let defaultsSuiteName = "remux-socket-password-migration-tests-\(UUID().uuidString)"
...
final class RemuxCLIPathInstallerTests: XCTestCase {
    func testInstallAndUninstallRoundTripWithoutAdministratorPrivileges() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("remux-cli-installer-tests-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let bundledCLIURL = root
            .appendingPathComponent("remux.app/Contents/Resources/bin/remux", isDirectory: false)
        ...
        let destinationURL = root.appendingPathComponent("usr/local/bin/remux", isDirectory: false)
