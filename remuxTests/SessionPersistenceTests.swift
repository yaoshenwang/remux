...
final class SessionPersistenceTests: XCTestCase {
    func testSaveAndLoadRoundTripWithCustomSnapshotPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-session-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRestorePolicySkipsWhenLaunchHasExplicitArguments() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/remux.app/Contents/MacOS/remux", "--window", "window:1"],
            environment: [:]
        )
    ...
    func testRestorePolicyAllowsFinderStyleLaunchArgumentsOnly() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/remux.app/Contents/MacOS/remux", "-psn_0_12345"],
            environment: [:]
        )
    ...
    func testRestorePolicySkipsWhenRunningUnderXCTest() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/remux.app/Contents/MacOS/remux"],
    ...
    func testScrollbackReplayEnvironmentWritesReplayFile() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-scrollback-replay-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/remux-screen.txt"),
            "/tmp/remux-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/remux-screen.txt "),
            "/tmp/remux-screen.txt"
        )
    }
    ...
    func testShouldRemoveExportedScreenDirectoryOnlyWithinTemporaryRoot() {
        let tempRoot = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("remux-export-tests-\(UUID().uuidString)", isDirectory: true)
...
final class RemuxAgentCommandTests: XCTestCase {
    func testAttachCommandShellQuotesTaggedAppPath() {
        let command = RemuxAgent.attachCommand(
            agentPath: "/Applications/remux DEV smoke-test.app/Contents/Resources/bin/remux-agent",
        ...
        XCTAssertEqual(
            command,
            "REMUX_AGENT_SOCKET='/tmp/remux smoke-test.sock' '/Applications/remux DEV smoke-test.app/Contents/Resources/bin/remux-agent' attach '00000000-0000-0000-0000-000000000000'"
        )
    }
