...
private var remuxUnitTestInspectorAssociationKey: UInt8 = 0
private var remuxUnitTestInspectorOverrideInstalled = false

private extension RemuxWebView {
    @objc func remuxUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &remuxUnitTestInspectorAssociationKey) as? NSObject
    }
}

private extension WKWebView {
    func remuxSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &remuxUnitTestInspectorAssociationKey,
            inspector,
...
private func installRemuxUnitTestInspectorOverride() {
    guard !remuxUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        RemuxWebView.self,
        #selector(RemuxWebView.remuxUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install RemuxWebView _inspector test override")
    }

    remuxUnitTestInspectorOverrideInstalled = true
}
...
final class RemuxWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = RemuxWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = RemuxWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = RemuxWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
...
final class AppDelegateWindowContextRoutingTests: XCTestCase {
    private func makeMainWindow(id: UUID) -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("remux.main.\(id.uuidString)")
        return window
    }
...
final class AppDelegateLaunchServicesRegistrationTests: XCTestCase {
    func testScheduleLaunchServicesRegistrationDefersRegisterWork() {
        ...
        app.scheduleLaunchServicesBundleRegistrationForTesting(
            bundleURL: URL(fileURLWithPath: "/tmp/../tmp/remux-launch-services-test.app"),
            scheduler: { work in
                scheduledWork = work
            },
            register: { _ in
...
@MainActor
final class RemuxWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = RemuxWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installRemuxUnitTestInspectorOverride()
    }

    private func makePanelWithInspector() -> (BrowserPanel, FakeInspector) {
        let panel = BrowserPanel(workspaceId: UUID())
        let inspector = FakeInspector()
        panel.webView.remuxSetUnitTestInspector(inspector)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(remuxOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(remuxOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(remuxOwningGhosttyView(for: view))
    }
}
...
final class CommandPaletteRenameSelectionSettingsTests: XCTestCase {
    private let suiteName = "remux.tests.commandPaletteRenameSelection.\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/yaoshenwang/remux/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/manaflow/remux/-/merge_requests/18")!,
            status: .merged
        )
...
final class SidebarBranchOrderingTests: XCTestCase {
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: pullRequestState(
                    number: 337,
                    label: "PR",
                    url: "https://github.com/yaoshenwang/remux/pull/337",
                    status: .open
                ),
                second: pullRequestState(
                    number: 18,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/remux/-/merge_requests/18",
                    status: .open
                ),
                ...
                fourth: pullRequestState(
                    number: 92,
                    label: "PR",
                    url: "https://bitbucket.org/manaflow/remux/pull-requests/92",
                    status: .closed
                )
            ],
        ...
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            ...
            panelPullRequests: [
                first: pullRequestState(
                    number: 42,
                    label: "PR",
                    url: "https://github.com/yaoshenwang/remux/pull/42",
                    status: .open
                ),
                second: pullRequestState(
                    number: 42,
                    label: "MR",
                    url: "https://gitlab.com/manaflow/remux/-/merge_requests/42",
                    status: .open
                )
            ],
    ...
    func testOrderedUniquePullRequestsTreatsSameNumberAndLabelDifferentUrlsAsDistinct() {
        ...
        XCTAssertEqual(
            ...
            [
                "https://github.com/yaoshenwang/remux/pull/42",
    ...
    func testOrderedUniquePullRequestsUsesFallbackWhenNoPanelPullRequestsExist() {
        let fallback = pullRequestState(
            number: 11,
            label: "PR",
            url: "https://github.com/yaoshenwang/remux/pull/11",
            status: .open
        )
        let pullRequests = SidebarBranchOrdering.orderedUniquePullRequests(
            orderedPanelIds: [],
...
final class FinderServicePathResolverTests: XCTestCase {
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/remux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/remux-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/remux-services/../remux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/remux-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/remux-services/project",
                "/tmp/remux-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/remux-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/remux-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/remux-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/remux-services/b/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/remux-services/b",
                "/tmp/remux-services/a",
            ]
        )
    }
}
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/remux"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/remux")
    }
...
final class NotificationDockBadgeTests: XCTestCase {
    ...
    func testNotificationCustomStagingPreservesSourceFileWithRemuxPrefix() {
        let suiteName = "NotificationDockBadgeTests.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated UserDefaults suite")
            return
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "remux-custom-notification-sound.source-\(UUID().uuidString).wav",
            isDirectory: false
        )
        defer {
        ...
        XCTAssertTrue(stagedName.hasPrefix("remux-custom-notification-sound-"))
    ...
    func testNotificationCustomUnsupportedExtensionsStageAsCaf() {
        ...
        XCTAssertTrue(stagedA.hasPrefix("remux-custom-notification-sound-"))
    ...
    func testNotificationCustomPreparationKeepsActiveSourceMetadataSidecar() {
        ...
        let sourceURL = soundsDirectory.appendingPathComponent(
            "remux-custom-notification-sound.metadata-\(UUID().uuidString).wav",
            isDirectory: false
        )
        do {
            try Data("test".utf8).write(to: sourceURL, options: .atomic)
    ...
    func testNotificationCustomSoundReturnsNilWhenPreparationFails() {
        ...
        let invalidSourceURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-invalid-sound-\(UUID().uuidString).mp3", isDirectory: false)
        defer {
            try? FileManager.default.removeItem(at: invalidSourceURL)
            let stagedURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent("Library", isDirectory: true)
                .appendingPathComponent("Sounds", isDirectory: true)
                .appendingPathComponent("remux-custom-notification-sound.caf", isDirectory: false)
    ...
    func testNotificationCustomPreparationReportsMissingFile() {
        let missingPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-missing-\(UUID().uuidString).wav", isDirectory: false)
            .path
...
final class MenuBarBuildHintFormatterTests: XCTestCase {
    func testReleaseBuildShowsNoHint() {
        XCTAssertNil(MenuBarBuildHintFormatter.menuTitle(appName: "remux DEV menubar-extra", isDebugBuild: false))
    }

    func testDebugBuildWithTagShowsTag() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "remux DEV menubar-extra", isDebugBuild: true),
    ...
    func testDebugBuildWithoutTagShowsUntagged() {
        XCTAssertEqual(
            MenuBarBuildHintFormatter.menuTitle(appName: "remux DEV", isDebugBuild: true),
            "Build: DEV (untagged)"
        )
    }
}
...
@MainActor
final class RemuxWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            RemuxWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(RemuxWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToRemuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToRemuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToRemuxBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInRemuxBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInRemuxBrowserValue(defaults: defaults))
    }
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/remux.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/remux.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/remux-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/remux-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/remux.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/remux-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/remux-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/remux-local-test.html"))
...
final class TerminalControllerSidebarDedupeTests: XCTestCase {
    ...
    func testNormalizeReportedDirectoryTrimsWhitespace() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("   /Users/remux/project   "),
            "/Users/remux/project"
        )
    }
    ...
    func testNormalizeReportedDirectoryResolvesFileURL() {
        XCTAssertEqual(
            TerminalController.normalizeReportedDirectory("file:///Users/remux/project"),
            "/Users/remux/project"
        )
    }
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    private func makeTempSocketPath() -> String {
        "/tmp/remux-socket-health-\(UUID().uuidString).sock"
    }
