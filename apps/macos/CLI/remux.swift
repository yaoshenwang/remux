...
private func currentProcessEnvironmentValue(keys: [String]) -> String? {
    firstNonEmptyEnvironmentValue(ProcessInfo.processInfo.environment, keys: keys)
}
...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = firstNonEmptyEnvironmentValue(
            ProcessInfo.processInfo.environment,
            keys: ["REMUX_BUNDLE_ID", "REMUX_BUNDLE_ID"]
        ) {
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = firstNonEmptyEnvironmentValue(processEnv, keys: [
            "REMUX_SOCKET_PATH", "REMUX_SOCKET", "REMUX_SOCKET_PATH", "REMUX_SOCKET"
        ])
        self.workspaceId = firstNonEmptyEnvironmentValue(processEnv, keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])
        self.surfaceId = firstNonEmptyEnvironmentValue(processEnv, keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"])
        self.disabledByEnv =
            firstNonEmptyEnvironmentValue(processEnv, keys: ["REMUX_CLI_SENTRY_DISABLED", "REMUX_CLI_SENTRY_DISABLED"]) == "1" ||
            firstNonEmptyEnvironmentValue(processEnv, keys: ["REMUX_CLAUDE_HOOK_SENTRY_DISABLED", "REMUX_CLAUDE_HOOK_SENTRY_DISABLED"]) == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "remux.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "remux-cli", key: "component")
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if !tmpSockets.isEmpty {
            context["tmp_remux_sockets"] = tmpSockets
        }
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "REMUX_SOCKET_PATH/REMUX_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func discoverSockets(in directory: String, limit: Int) -> [String] {
        ...
        for name in entries.sorted() {
            guard name.hasPrefix("remux"), name.hasSuffix(".sock") else { continue }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.remux/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = firstNonEmptyEnvironmentValue(
            processEnv,
            keys: ["REMUX_CLAUDE_HOOK_STATE_PATH", "REMUX_CLAUDE_HOOK_STATE_PATH"]
        ),
...
enum SocketPasswordResolver {
    private static let service = "com.remux.macos.socket-control"
    private static let legacyService = "com.remux.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "remux"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = firstNonEmptyEnvironmentValue(
            ProcessInfo.processInfo.environment,
            keys: ["REMUX_SOCKET_PASSWORD", "REMUX_SOCKET_PASSWORD"]
        ) {
            return env
        }
    ...
    private static func keychainScope(
        ...
        if let tag = firstNonEmptyEnvironmentValue(environment, keys: ["REMUX_TAG", "REMUX_TAG"]) {
        ...
        let prefixes = ["remux-debug-", "remux-", "remux-debug-", "remux-"]
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "remux"
    private static let stableSocketFileName = "remux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/remux.sock"
    private static let fallbackSocketPath = "/tmp/remux-debug.sock"
    private static let stagingSocketPath = "/tmp/remux-staging.sock"
    private static let legacyRemuxDefaultSocketPath = "/tmp/remux.sock"
    private static let legacyRemuxFallbackSocketPath = "/tmp/remux-debug.sock"
    private static let legacyRemuxStagingSocketPath = "/tmp/remux-staging.sock"
    private static let currentLastSocketPathFile = "/tmp/remux-last-socket-path"
    private static let legacyLastSocketPathFile = "/tmp/remux-last-socket-path"
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = firstNonEmptyEnvironmentValue(environment, keys: ["REMUX_TAG", "REMUX_TAG"]) {
            ...
            candidates.append("/tmp/remux-debug-\(slug).sock")
            candidates.append("/tmp/remux-\(slug).sock")
            candidates.append("/tmp/remux-debug-\(slug).sock")
            candidates.append("/tmp/remux-\(slug).sock")
        }
        ...
        candidates.append(legacyRemuxDefaultSocketPath)
        candidates.append(legacyRemuxFallbackSocketPath)
        candidates.append(legacyRemuxStagingSocketPath)
    ...
    private static func readLastSocketPath() -> String? {
        let primaryCandidate: String? = stableSocketDirectoryURL()?
            ...
            .path
        let candidates = [primaryCandidate, currentLastSocketPathFile, legacyLastSocketPathFile].compactMap { $0 }
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where (name.hasPrefix("remux") || name.hasPrefix("remux")) && name.hasSuffix(".sock") {
                ...
                if [
                    ...
                    stagingSocketPath,
                    legacyRemuxDefaultSocketPath,
                    legacyRemuxFallbackSocketPath,
                    legacyRemuxStagingSocketPath,
                ].contains(path) {
                    continue
                }
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["REMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "remux app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.remux.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.remux.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct REMUXCLI {
    ...
    private static let debugLastSocketHintPaths = [
        "/tmp/remux-last-socket-path",
        "/tmp/remux-last-socket-path",
    ]
    private static let primaryCLIName = "remux"
...
#if DEBUG
        for hintPath in debugLastSocketHintPaths {
            guard let raw = try? String(contentsOfFile: hintPath, encoding: .utf8) else {
                continue
            }
            guard let hinted = normalizedEnvValue(raw),
                  (hinted.hasPrefix("/tmp/remux-debug") || hinted.hasPrefix("/tmp/remux-debug")),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = firstNonEmptyEnvironmentValue(environment, keys: ["REMUX_SOCKET_PATH", "REMUX_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
    ...
    private func presentCLIText(_ text: String) -> String {
        var rendered = text
        let replacements: [(String, String)] = [
            ("Usage: remux", "Usage: remux"),
            ("Run 'remux", "Run 'remux"),
            ("`remux ", "`remux "),
            ("control remux via Unix socket", "control Remux via Unix socket"),
            ("launches remux", "launches Remux"),
            ("running remux", "running Remux"),
            ("inside remux", "inside Remux"),
            ("remux terminals", "Remux terminals"),
            ("~/Library/Application Support/remux/remux.sock", "~/Library/Application Support/remux/remux.sock"),
            ("$REMUX_", "$REMUX_"),
            ("REMUX_SOCKET_PASSWORD", "REMUX_SOCKET_PASSWORD"),
        ]
        for (source, target) in replacements {
            rendered = rendered.replacingOccurrences(of: source, with: target)
        }
        return rendered
    }
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["REMUX_SOCKET_PATH", "REMUX_SOCKET", "REMUX_SOCKET_PATH", "REMUX_SOCKET"] {
                if let value = firstNonEmptyEnvironmentValue(processEnv, keys: [key]) {
                    return value
                }
            }
            return nil
        ...
        // so help text is available even when the app is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run '\(Self.primaryCLIName) help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside remux
        // (before socket connection, so it doesn't fail when no socket exists)
        if command == "codex-hook" {
            guard currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) != nil else {
                print("{}")
                return
            }
        }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
            ...
            let callerSurfaceArg = explicitSurfaceArg == nil && preferTTYFallback == false && windowId == nil
                ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"])
                : nil
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]),
        ...
        // Remux commands
        case "remux":
            try runRemuxCommand(commandArgs: commandArgs, client: client, jsonOutput: jsonOutput)
        case "server":
            try runRemuxCommand(commandArgs: commandArgs, client: client, jsonOutput: jsonOutput)
        case "connect":
            try runRemuxCommand(commandArgs: ["connect"] + commandArgs, client: client, jsonOutput: jsonOutput)
        case "history":
            try runRemuxCommand(commandArgs: ["history"] + commandArgs, client: client, jsonOutput: jsonOutput)
        case "home":
            try runRemuxCommand(commandArgs: ["home"] + commandArgs, client: client, jsonOutput: jsonOutput)
    ...
    private func runRemuxCommand(
        commandArgs: [String],
        client: SocketClient,
        jsonOutput: Bool
    ) throws {
        let first = commandArgs.first ?? "status"
        let subcommand: String
        let subArgs: [String]
        if first == "server" {
            subcommand = commandArgs.dropFirst().first ?? "status"
            subArgs = Array(commandArgs.dropFirst(2))
        } else {
            subcommand = first
        ...
        switch subcommand {
        case "status":
            let result = try client.sendV2(method: "remux.server.status", params: [:])
            if jsonOutput {
                print(jsonString(result))
            } else {
                printRemuxStatus(result)
            }

        case "start":
            let result = try client.sendV2(method: "remux.server.start", params: [:])
            if jsonOutput {
                print(jsonString(result))
            } else {
        ...
        case "stop":
            let result = try client.sendV2(method: "remux.server.stop", params: [:])
            if jsonOutput {
                print(jsonString(result))
            } else {
        ...
        case "restart":
            let result = try client.sendV2(method: "remux.server.restart", params: [:])
            if jsonOutput {
                print(jsonString(result))
            } else {
        ...
        case "connect":
            ...
            while i < subArgs.count {
                ...
                case "--token", "-t":
                    i += 1
                    if i < subArgs.count { token = subArgs[i] }
                case "--label", "-l":
                    i += 1
                    if i < subArgs.count { label = subArgs[i] }
                default:
                    if target == nil { target = subArgs[i] }
                }
                i += 1
            }
            guard let t = target else {
                fputs("Usage: remux connect <url-or-code> [--token <token>] [--label <name>]\n", stderr)
                throw CLIError(message: "target required: remux connect <url-or-code> [--token <token>]")
            }
            var params: [String: Any] = ["target": t]
            if let tok = token, !tok.isEmpty {
                params["token"] = tok
            }
            if let l = label { params["label"] = l }
            let result = try client.sendV2(method: "remux.connect", params: params)
            if jsonOutput {
                print(jsonString(result))
            } else if let ws = (result as? [String: Any])?["workspace_id"] as? String {
                print("Connected: workspace \(ws)")
            }

        case "history":
            let historySubcommand = subArgs.first ?? "list"
            switch historySubcommand {
            case "list":
                let result = try client.sendV2(method: "remux.history.list", params: [:])
                if jsonOutput {
                    print(jsonString(result))
                } else {
                    printRemuxHistory(result)
                }
            case "connect":
                guard let query = subArgs.dropFirst().first, !query.isEmpty else {
                    fputs("Usage: remux history connect <id|label|url>\n", stderr)
                    throw CLIError(message: "query required: remux history connect <id|label|url>")
                }
                let result = try client.sendV2(method: "remux.history.connect", params: ["query": query])
                if jsonOutput {
                    print(jsonString(result))
                } else if let ws = (result as? [String: Any])?["workspace_id"] as? String {
                    print("Connected from history: workspace \(ws)")
                }
            case "remove", "rm", "delete":
                guard let query = subArgs.dropFirst().first, !query.isEmpty else {
                    fputs("Usage: remux history remove <id|label|url>\n", stderr)
                    throw CLIError(message: "query required: remux history remove <id|label|url>")
                }
                let result = try client.sendV2(method: "remux.history.remove", params: ["query": query])
                if jsonOutput {
                    print(jsonString(result))
                } else {
                    let dict = result as? [String: Any]
                    let label = dict?["label"] as? String ?? query
                    print("Removed history entry: \(label)")
                }
            case "clear":
                var params: [String: Any] = [:]
                if let id = subArgs.dropFirst().first { params["id"] = id }
                let _ = try client.sendV2(method: "remux.history.clear", params: params)
                print("History cleared")
            default:
                fputs("Unknown history subcommand: \(historySubcommand)\n", stderr)
                fputs("Usage: remux history [list|connect|remove|clear]\n", stderr)
            }

        case "home":
            let _ = try client.sendV2(method: "remux.home", params: [:])
            print("Showing home panel")

        case "help", "--help", "-h":
            print("""
            remux — Remux macOS client commands

            SUBCOMMANDS:
              server [status]     Show embedded server status (ID, token, tunnel URL)
              server start        Start the embedded Remux server
              server stop         Stop the embedded Remux server
              server restart      Restart the embedded Remux server
              connect <target>    Connect to a local/remote Remux server or 9-digit ID
                --token, -t       Authentication token (optional for short-code flows)
                --label, -l       Display label for the connection
              history list         List saved connections
              history connect <q> Reconnect using a saved history entry
              history remove <q>  Remove one saved history entry
              history clear       Clear all saved history
              home                Show the Remux home panel
            """)

        default:
            fputs("Unknown remux subcommand: \(subcommand)\n", stderr)
            fputs("Run 'remux help' for usage\n", stderr)
        }
    }

    private func printRemuxStatus(_ result: Any) {
        ...
        print("Remux Server: \(state)")
        print("Port: \(port)")
        if let url = dict["local_url"] as? String { print("Local: \(url)") }
        if let tunnel = dict["tunnel_url"] as? String { print("Tunnel: \(tunnel)") }
        if let code = dict["short_code"] as? String { print("ID: \(code)") }
        if let token = dict["token"] as? String { print("Token: \(token)") }
    }

    private func printRemuxHistory(_ result: Any) {
        ...
        for entry in entries {
            ...
            let date = entry["last_connected"] as? String ?? "?"
            print("\(id)  \(label)  \(url)  (\(date))")
        }
    }
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: remux markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: remux markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
        if let workspaceRaw {
    ...
    /// Open a path in remux by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = firstNonEmptyEnvironmentValue(env, keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (currentProcessEnvironmentValue(keys: ["REMUX_TAB_ID", "REMUX_TAB_ID"]) ?? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]))
                : nil)
    ...
    private func runSSH(
        ...
        do {
            ...
            // `remux ssh` is an explicit "open this remote workspace now" action,
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: remux ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        let remoteCommandBase64Placeholder = "__REMUX_REMOTE_BOOTSTRAP_B64_RUNTIME__"
        ...
        var lines: [String] = [
            "remux_workspace_id=\"${REMUX_WORKSPACE_ID:-}\"",
            "remux_surface_id=\"${REMUX_SURFACE_ID:-}\"",
            "remux_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "remux_remote_bootstrap=\"$(printf %s \"$remux_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$remux_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "remux_remote_bootstrap=\"$(printf '%s' \"$remux_remote_bootstrap\" | sed \"s/__REMUX_WORKSPACE_ID__/$remux_workspace_id/g; s/__REMUX_SURFACE_ID__/$remux_surface_id/g\")\"",
            "remux_remote_bootstrap_b64_runtime=\"$(printf '%s' \"$remux_remote_bootstrap\" | base64 | tr -d '\\n')\"",
            "remux_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "remux_remote_command=\"$(printf '%s' \"$remux_remote_command_template\" | sed \"s|\(remoteCommandBase64Placeholder)|$remux_remote_bootstrap_b64_runtime|g\")\"",
        ]

        var sshInvocation = "command \(sshPrefix) -o \"RemoteCommand=$remux_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(base64Placeholder: String) -> String {
        return [
            "remux_tmp=$(mktemp \"${TMPDIR:-/tmp}/remux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$remux_tmp\" || { rm -f \"$remux_tmp\"; exit 1; }",
            "chmod 700 \"$remux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$remux_tmp\"",
            "remux_status=$?",
            "rm -f \"$remux_tmp\"",
            "exit $remux_status",
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__REMUX_WORKSPACE_ID__' ]; then export REMUX_WORKSPACE_ID='__REMUX_WORKSPACE_ID__'; fi",
            "if [ -n '__REMUX_SURFACE_ID__' ]; then export REMUX_SURFACE_ID='__REMUX_SURFACE_ID__'; fi",
        ]
        let relaySocket = remoteRelayPort > 0 ? "127.0.0.1:\(remoteRelayPort)" : nil
        let shellStateDir = "$HOME/.remux/relay/\(max(remoteRelayPort, 0)).shell"
        var commonShellLines = remoteTerminalLines
        commonShellLines.append(contentsOf: remoteEnvExportLines)
        commonShellLines.append("export PATH=\"$HOME/.remux/bin:$PATH\"")
        if let relaySocket {
            commonShellLines.append("export REMUX_SOCKET_PATH=\(relaySocket)")
        }
        ...
        var outerLines: [String] = [
            "REMUX_LOGIN_SHELL=\"${SHELL:-/bin/zsh}\"",
            "case \"${REMUX_LOGIN_SHELL##*/}\" in",
            "  zsh)",
            "    mkdir -p \"$HOME/.remux/relay\"",
            "    remux_shell_dir=\"\(shellStateDir)\"",
            "    mkdir -p \"$remux_shell_dir\"",
            "    cat > \"$remux_shell_dir/.zshenv\" <<'REMUXZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "REMUXZSHENV",
            "    cat > \"$remux_shell_dir/.zprofile\" <<'REMUXZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "REMUXZSHPROFILE",
            "    cat > \"$remux_shell_dir/.zshrc\" <<'REMUXZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "REMUXZSHRC",
            "    cat > \"$remux_shell_dir/.zlogin\" <<'REMUXZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "REMUXZSHLOGIN",
            "    chmod 600 \"$remux_shell_dir/.zshenv\" \"$remux_shell_dir/.zprofile\" \"$remux_shell_dir/.zshrc\" \"$remux_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export REMUX_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$remux_shell_dir\"",
            "    exec \"$REMUX_LOGIN_SHELL\" -il",
            "    ;;",
            "  bash)",
            "    mkdir -p \"$HOME/.remux/relay\"",
            "    remux_shell_dir=\"\(shellStateDir)\"",
            "    mkdir -p \"$remux_shell_dir\"",
            "    cat > \"$remux_shell_dir/.bashrc\" <<'REMUXBASHRC'",
        ]
        outerLines.append(contentsOf: bashRCLines)
        outerLines += [
            "REMUXBASHRC",
            "    chmod 600 \"$remux_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$REMUX_LOGIN_SHELL\" --rcfile \"$remux_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
    ...
    func encodedRemoteBootstrapCommand(_ remoteBootstrapScript: String) -> String {
        ...
        return [
            "remux_tmp=$(mktemp \"${TMPDIR:-/tmp}/remux-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$remux_tmp\" || { rm -f \"$remux_tmp\"; exit 1; }",
            "chmod 700 \"$remux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$remux_tmp\"",
            "remux_status=$?",
            "rm -f \"$remux_tmp\"",
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            "REMUX_SSH_SESSION_ENDED=0",
            "remux_ssh_session_end() { if [ \"${REMUX_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; REMUX_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'remux_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
        ...
        scriptLines += [
            ...
            "remux_ssh_session_end",
            "exit $remux_ssh_status",
        ]
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "remux-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${REMUX_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${REMUX_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${REMUX_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${REMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${REMUX_SURFACE_ID:-}\" ]; then",
            "\"${REMUX_BUNDLED_CLI_PATH}\" --socket \"${REMUX_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${REMUX_WORKSPACE_ID}\" --surface \"${REMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v remux >/dev/null 2>&1",
            "&& [ -n \"${REMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${REMUX_SURFACE_ID:-}\" ]; then",
            "remux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${REMUX_WORKSPACE_ID}\" --surface \"${REMUX_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"])
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or REMUX_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or REMUX_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let payload: [String: Any] = [
            ...
            "commit": info["REMUXCommit"] ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["REMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set REMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["REMUXRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("remux-remote-daemons", isDirectory: true)
                .appendingPathComponent(version, isDirectory: true)
                .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
                .appendingPathComponent("remuxd-remote", isDirectory: false)
        }
        return root
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/remux-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/remux-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = currentProcessEnvironmentValue(keys: ["REMUX_DEBUG_LOG", "REMUX_DEBUG_LOG"])?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/remux-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [remux-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'remux browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__remux_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = currentProcessEnvironmentValue(keys: ["REMUX_RESPECT_EXTERNAL_OPEN_RULES", "REMUX_RESPECT_EXTERNAL_OPEN_RULES"]) else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("remux-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the remux socket server.
            """
        case "capabilities":
            return """
            Usage: remux capabilities

            Print server capabilities as JSON.
            """
        case "server":
            return """
            Usage: remux server [status|start|stop|restart]

            Manage the embedded Remux server used by the macOS client.
            """
        case "connect":
            return """
            Usage: remux connect <url-or-code> [--token <token>] [--label <name>]

            Connect to a local or remote Remux server. 9-digit connection IDs
            are resolved through the discovery service and may auto-fill the token.
            """
        case "history":
            return """
            Usage: remux history [list|connect|remove|clear]

            List, reconnect, remove, or clear recent Remux connections stored by the macOS client.
            """
        case "home":
            return """
            Usage: remux home

            Show the Remux home panel in the running macOS client.
            """
        case "help":
            return """
            Usage: remux help
        ...
        case "welcome":
            ...
            Show a welcome screen with the remux logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: remux shortcuts

            Open the Settings window to Keyboard Shortcuts.
            """
        case "feedback":
            return """
            Usage: remux feedback
                   remux feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            ...
            Usage: remux themes
                   remux themes list
                   remux themes set <theme>
                   remux themes set --light <theme> [--dark <theme>]
                   remux themes set --dark <theme> [--light <theme>]
                   remux themes clear

            When run in a TTY, `remux themes` opens an interactive theme picker with
            live app preview. Use `remux themes list` for a plain listing.

            The picker previews the selected theme across the running remux app and
            ...
            Commands:
              ...
              clear                     Remove the remux theme override and fall back to other config

            Examples:
              remux themes
              remux themes list
              remux themes set "Catppuccin Mocha"
              remux themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              remux themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: remux claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses remux splits
            ...
            The tmux shim translates supported tmux window/pane commands into remux
            workspace and split operations in the current remux session.

            Examples:
              remux claude-teams
              remux claude-teams --continue
              remux claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: remux omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in a remux-aware environment.
            ...
            remux splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses remux splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into remux workspace
            and split operations in the current remux session.

            Examples:
              remux omo
              remux omo --continue
              remux omo --model claude-sonnet-4-6
            """)
        case "identify":
            return """
            Usage: remux identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $REMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $REMUX_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: remux list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: remux current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: remux new-window
        ...
        case "focus-window":
            return """
            Usage: remux focus-window --window <id|ref|index>
            ...
            Example:
              remux focus-window --window 0
              remux focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: remux close-window --window <id|ref|index>
            ...
            Example:
              remux close-window --window 0
              remux close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: remux move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              remux move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: remux move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              remux move-surface --surface surface:1 --workspace workspace:2
              remux move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: remux reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              remux reorder-surface --surface surface:1 --index 0
              remux reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: remux reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              remux reorder-workspace --workspace workspace:2 --index 0
              remux reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: remux workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$REMUX_WORKSPACE_ID)
            ...
            Example:
              remux workspace-action --workspace workspace:2 --action pin
              remux workspace-action --action rename --title "infra"
              remux workspace-action close-others
              remux workspace-action --action set-color --color blue
              remux workspace-action --action set-color --color "#C0392B"
              remux workspace-action set-color Amber
              remux workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: remux tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $REMUX_TAB_ID, then $REMUX_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$REMUX_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              remux tab-action --tab tab:3 --action pin
              remux tab-action --action close-right
              remux tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: remux rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            1) --tab
            2) --surface
            3) $REMUX_TAB_ID / $REMUX_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$REMUX_WORKSPACE_ID)
            ...
            Examples:
              remux rename-tab "build logs"
              remux rename-tab --tab tab:3 "staging server"
              remux rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: remux new-workspace [--name <title>] [--cwd <path>] [--command <text>]
            ...
            Example:
              remux new-workspace
              remux new-workspace --name "Build Server"
              remux new-workspace --cwd ~/projects/myapp
              remux new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: remux list-workspaces
        ...
        case "ssh":
            return """
            Usage: remux ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            remux will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              remux ssh dev@my-host
              remux ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              remux ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: remux remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded remuxd-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              remux remote-daemon-status
              remux remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: remux new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $REMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $REMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              remux new-split right
              remux new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: remux list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $REMUX_WORKSPACE_ID)

            Example:
              remux list-panes
              remux list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: remux list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              remux list-pane-surfaces
              remux list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: remux tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `remux tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              remux tree
              remux tree --all
              remux tree --workspace workspace:2
              remux --json tree --all
            """
        case "focus-pane":
            return """
            Usage: remux focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              remux focus-pane pane:1
              remux focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: remux new-pane [flags]
            ...
            Example:
              remux new-pane
              remux new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: remux new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: remux close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $REMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $REMUX_WORKSPACE_ID)

            Example:
              remux close-surface
              remux close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: remux drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              remux drag-surface-to-split --surface surface:1 right
              remux drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: remux refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "surface-health":
            return """
            Usage: remux surface-health [--workspace <id|ref>]
        ...
        case "trigger-flash":
            return """
            Usage: remux trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $REMUX_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $REMUX_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              remux trigger-flash
              remux trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: remux list-panels [--workspace <id|ref>]
            ...
            Example:
              remux list-panels
              remux list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: remux focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              remux focus-panel --panel surface:2
              remux focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: remux close-workspace --workspace <id|ref|index>
            ...
            Example:
              remux close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: remux select-workspace --workspace <id|ref|index>
            ...
            Example:
              remux select-workspace --workspace workspace:2
              remux select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: remux rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$REMUX_WORKSPACE_ID)

            Example:
              remux rename-workspace "backend logs"
              remux rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: remux current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: remux capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $REMUX_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $REMUX_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              remux capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: remux resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: remux pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: remux wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: remux swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: remux break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: remux join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: remux last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: remux find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: remux clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: remux set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: remux set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: remux paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: remux list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: remux respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: remux display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              remux read-screen
              remux read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: remux send [flags] [--] <text>
            ...
            Example:
              remux send "echo hello"
              remux send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: remux send-key [flags] [--] <key>
            ...
            Example:
              remux send-key enter
              remux send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: remux send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              remux send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: remux send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              remux send-key-panel --panel surface:2 enter
              remux send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: remux notify [flags]
            ...
            Example:
              remux notify --title "Build done" --body "All tests passed"
              remux notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: remux list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: remux clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: remux set-status <key> <value> [flags]
            ...
            Example:
              remux set-status build "compiling" --icon hammer --color "#ff9500"
              remux set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: remux clear-status <key> [flags]
            ...
            Example:
              remux clear-status build
            """
        case "list-status":
            return """
            Usage: remux list-status [flags]
            ...
            Example:
              remux list-status
              remux list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: remux set-progress <0.0-1.0> [flags]
            ...
            Example:
              remux set-progress 0.5 --label "Building..."
              remux set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: remux clear-progress [flags]
            ...
            Example:
              remux clear-progress
            """
        case "log":
            return """
            Usage: remux log [flags] [--] <message>
        ...
        case "sidebar-state":
            return """
            Usage: remux sidebar-state [flags]
        ...
        case "set-app-focus":
            return """
            Usage: remux set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              remux set-app-focus inactive
              remux set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: remux simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: remux claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | remux claude-hook session-start
              echo '{}' | remux claude-hook stop
            """
        case "codex":
            return """
            Usage: remux codex <install-hooks|uninstall-hooks>

            Manage Codex CLI hooks integration.

            Subcommands:
              install-hooks     Install remux hooks into ~/.codex/hooks.json
              uninstall-hooks   Remove remux hooks from ~/.codex/hooks.json
            """
        case "codex-hook":
            return """
            Usage: remux codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside remux.
        ...
        case "browser":
            return """
            Usage: remux browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $REMUX_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              ...
              remux browser surface:1 navigate https://google.com
              remux browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `remux browser --help`
        case "open-browser":
            return "Legacy alias for 'remux browser open'. Run 'remux browser --help' for details."
        case "navigate":
            return "Legacy alias for 'remux browser navigate'. Run 'remux browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'remux browser back'. Run 'remux browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'remux browser forward'. Run 'remux browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'remux browser reload'. Run 'remux browser --help' for details."
        case "get-url":
            return "Legacy alias for 'remux browser get-url'. Run 'remux browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'remux browser focus-webview'. Run 'remux browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'remux browser is-webview-focused'. Run 'remux browser --help' for details."
        case "markdown":
            return """
            Usage: remux markdown open <path> [options]
                   remux markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $REMUX_WORKSPACE_ID)
              --surface <id|ref|index>     Source surface to split from (default: focused surface)
              --window <id|ref|index>      Target window
              --direction <left|right|up|down>  Split direction (default: right)

            Examples:
              remux markdown open plan.md
              remux markdown ~/project/CHANGELOG.md
              remux markdown open ./docs/design.md --workspace 0
              remux markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private func dispatchSubcommandHelp(command: String, commandArgs: [String]) -> Bool {
        ...
        print("\(Self.primaryCLIName) \(command)")
        print("")
        print(presentCLIText(text))
        return true
    }

    private static let remuxThemeOverrideBundleIdentifier = "com.remux.app"
    private static let remuxThemesBlockStart = "# remux themes start"
    private static let remuxThemesBlockEnd = "# remux themes end"
    private static let remuxThemesReloadNotificationName = "com.remux.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["REMUX_THEME_PICKER_CONFIG"] = try remuxThemeOverrideConfigURL().path
        environment["REMUX_THEME_PICKER_BUNDLE_ID"] = currentRemuxAppBundleIdentifier() ?? Self.remuxThemeOverrideBundleIdentifier
        environment["REMUX_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["REMUX_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["REMUX_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["REMUX_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'remux themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try remuxThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'remux themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.remuxThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.remuxThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func remuxThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.remuxThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try remuxThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.remuxThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.remuxThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentRemuxAppBundleIdentifier() ?? Self.remuxThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.remuxThemesReloadNotificationName),
            object: nil,
    ...
    private func currentRemuxAppBundleIdentifier() -> String? {
        if let bundleIdentifier = currentProcessEnvironmentValue(keys: ["REMUX_BUNDLE_ID", "REMUX_BUNDLE_ID"])?.trimmingCharacters(in: .whitespacesAndNewlines),
           !bundleIdentifier.isEmpty {
            return bundleIdentifier
        }
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = firstNonEmptyEnvironmentValue(env, keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = firstNonEmptyEnvironmentValue(env, keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"])?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]))
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["REMUX_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]))
    }
    ...
    private func tmuxResolveSurfaceTarget(
        ...
        if tmuxPaneSelector(from: raw) != nil {
            ...
            // exact surface (REMUX_SURFACE_ID) over the pane's currently selected
    ...
    private func tmuxCompatResolvedSocketPath(processEnvironment: [String: String]) -> String {
        let envSocketPath: String? = {
            for key in ["REMUX_SOCKET_PATH", "REMUX_SOCKET"] {
    ...
    private func isRemuxClaudeWrapper(at path: String) -> Bool {
        guard let data = FileManager.default.contents(atPath: path) else { return false }
        let prefixData = data.prefix(512)
        guard let prefix = String(data: prefixData, encoding: .utf8) else { return false }
        return prefix.contains("remux claude wrapper - injects hooks and session tracking")
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isRemuxClaudeWrapper(at: $0) }
        )
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        if let focusedContext {
            setenv("REMUX_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("REMUX_SURFACE_ID", surfaceId, 1)
            }
        }
    }

    private static let claudeNodeOptionsRestoreModule = """
    const hadOriginalNodeOptions = process.env.REMUX_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
    if (hadOriginalNodeOptions) {
        process.env.NODE_OPTIONS = process.env.REMUX_ORIGINAL_NODE_OPTIONS ?? "";
    } else {
        delete process.env.NODE_OPTIONS;
    }
    delete process.env.REMUX_ORIGINAL_NODE_OPTIONS;
    delete process.env.REMUX_ORIGINAL_NODE_OPTIONS_PRESENT;
    """
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            tmuxPathPrefix: "remux-claude-teams",
            remuxBinEnvVar: "REMUX_CLAUDE_TEAMS_REMUX_BIN",
            termOverrideEnvVar: "REMUX_CLAUDE_TEAMS_TERM",
            extraEnvVars: [
                (key: "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", value: "1"),
            ]
        )
        guard let restoreModuleURL = try? createClaudeNodeOptionsRestoreModule() else {
            unsetenv("REMUX_ORIGINAL_NODE_OPTIONS_PRESENT")
            unsetenv("REMUX_ORIGINAL_NODE_OPTIONS")
            return
        }
        if let existing = processEnvironment["NODE_OPTIONS"] {
            setenv("REMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "1", 1)
            setenv("REMUX_ORIGINAL_NODE_OPTIONS", existing, 1)
        } else {
            setenv("REMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "0", 1)
            unsetenv("REMUX_ORIGINAL_NODE_OPTIONS")
        }
        setenv(
            "NODE_OPTIONS",
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("remux-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["REMUX_SOCKET_PATH"] = socketPath
        launcherEnvironment["REMUX_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["REMUX_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "remux")
        ...
        let claudeExecutablePath: String? = {
            ...
            let candidates = [
                launcherEnvironment["REMUX_CUSTOM_CLAUDE_PATH"],
            ...
            for raw in candidates {
                ...
                guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDir),
                      ...
                      !isRemuxClaudeWrapper(at: trimmed) else { continue }
                return trimmed
            }
    ...
    // MARK: - remux omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to remux __tmux-compat
        ...
        exec "${REMUX_OMO_REMUX_BIN:-remux}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to remux notify
        let notifierURL = root.appendingPathComponent("terminal-notifier", isDirectory: false)
        let notifierScript = """
        #!/usr/bin/env bash
        # Intercept terminal-notifier calls and route through remux notify.
        ...
        exec "${REMUX_OMO_REMUX_BIN:-remux}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private func configureOMOEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "remux-omo",
            remuxBinEnvVar: "REMUX_OMO_REMUX_BIN",
            termOverrideEnvVar: "REMUX_OMO_TERM",
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in remux claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in remux claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/remux-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"]) : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in remux CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in remux CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "remux", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? currentProcessEnvironmentValue(keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? currentProcessEnvironmentValue(keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["REMUX_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                remux claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["REMUX_CLI_TTY_NAME", "REMUX_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    /// The hooks.json content that remux installs into ~/.codex/.
    /// Each hook calls `remux codex-hook <event>` which gracefully no-ops
    /// when not running inside remux. The command checks for remux on PATH
    /// first so it silently succeeds even when remux is not installed
    /// (e.g. user opened codex in a non-remux terminal).
    private static func codexHookCommand(_ event: String) -> String {
        "[ -n \"$REMUX_SURFACE_ID\" ] && command -v remux >/dev/null 2>&1 && remux codex-hook \(event) || echo '{}'"
    }
    ...
    /// Identifier used to detect remux-owned hooks during uninstall.
    private static let codexHookCommandMarker = "remux codex-hook"
    ...
    private func runCodexInstallHooks() throws {
        ...
        let remuxHooks = Self.codexHooksJSON["hooks"] as! [String: Any]
        for (eventName, remuxGroups) in remuxHooks {
            guard let remuxGroupArray = remuxGroups as? [[String: Any]] else { continue }
            ...
            eventGroups.append(contentsOf: remuxGroupArray)
        ...
        if !hooksChanged && !configChanged {
            print("remux hooks are already installed. Nothing to change.")
            return
        }
        ...
        print("")
        print("Installed. Hooks activate inside remux and silently no-op elsewhere.")
        print("To remove: remux codex uninstall-hooks")
    }
    ...
    private func runCodexUninstallHooks() throws {
        ...
        // Build the new state without remux hooks
        var removedCount = 0
        ...
        print("Removed remux Codex hooks.")
    }
    ...
    /// Codex hook handler. Gracefully no-ops when not running inside remux.
    private func runCodexHook(
        ...
        // Graceful no-op: if not inside remux, exit silently with valid JSON
        guard firstNonEmptyEnvironmentValue(env, keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) != nil else {
            print("{}")
            return
        }
        ...
        let hookArgs = Array(commandArgs.dropFirst())
        let hookWsFlag = optionValue(hookArgs, name: "--workspace")
        let workspaceArg = hookWsFlag ?? firstNonEmptyEnvironmentValue(env, keys: ["REMUX_WORKSPACE_ID", "REMUX_WORKSPACE_ID"])
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? firstNonEmptyEnvironmentValue(env, keys: ["REMUX_SURFACE_ID", "REMUX_SURFACE_ID"]) : nil)
        let rawInput = String(data: FileHandle.standardInput.readDataToEndOfFile(), encoding: .utf8) ?? ""
        let parsedInput = parseClaudeHookInput(rawInput: rawInput)
        let sessionStore = ClaudeHookSessionStore(
            processEnv: env.merging(
                ["REMUX_CLAUDE_HOOK_STATE_PATH": "~/.remux/codex-hook-sessions.json"],
        ...
        case "help", "--help", "-h":
            print("remux codex-hook <session-start|prompt-submit|stop> [--workspace <id>] [--surface <id>]")
    ...
    private func versionSummary() -> String {
        let info = resolvedVersionInfo()
        let commit = info["REMUXCommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "remux \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "remux \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "remux build \(build)"
        } else {
            baseSummary = "remux version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print("  \(subdued)Run \(reset)\(bold)remux shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)remux feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["REMUXCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["REMUX_COMMIT"]) {
            info["REMUXCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["REMUXCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["REMUXCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return presentCLIText("""
        remux - control remux via Unix socket

        Usage:
          remux <path>                Open a directory in a new workspace (launches remux if needed)
          remux [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then REMUX_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Commands:
          ...
          connect <url-or-code> [--token <token>] [--label <name>]
          history [list|connect|remove|clear]
          home
        ...
        Environment:
          REMUX_WORKSPACE_ID   Auto-set in remux terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          REMUX_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          REMUX_SURFACE_ID     Auto-set in remux terminals. Used as default --surface.
          REMUX_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/remux/remux.sock and auto-discovers tagged/debug sockets.
        """)
    }
