...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/remux-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/remux-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["REMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["REMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["REMUX_SOCKET_PATH"] = socketPath
        app.launchEnvironment["REMUX_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["REMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["REMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "remux_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["REMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["REMUX_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveRemuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled remux CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `remux notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected remux to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `remux notify` command to finish without foregrounding remux. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected remux to remain in background after bundled `remux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `remux notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForRemuxPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runRemuxCommand(
        ...
        let result = runRemuxCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runRemuxCommand(
        ...
        cliStrategy: RemuxCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["REMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveRemuxCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled remux CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeRemuxCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled remux CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["remux"] + args
        let fallbackResult = executeRemuxCommand(
    ...
    private enum RemuxCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveRemuxCLIPaths(strategy: RemuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["REMUX_UI_TEST_CLI_PATH", "REMUXTERM_CLI"] {
        ...
        candidates.append("/tmp/remux-\(launchTag)/Build/Products/Debug/remux DEV.app/Contents/Resources/bin/remux")
        candidates.append("/tmp/remux-\(launchTag)/Build/Products/Debug/remux.app/Contents/Resources/bin/remux")
        if strategy == .any {
            candidates.append("/tmp/remux-\(launchTag)/Build/Products/Debug/remux")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: RemuxCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/remux DEV.app/Contents/Resources/bin/remux")
        candidates.append("\(productsDir)/remux.app/Contents/Resources/bin/remux")
        if strategy == .any {
            candidates.append("\(productsDir)/remux")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/remux")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "remux" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeRemuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run remux command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested REMUX_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/remux-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/remux-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/remux.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("remux.sock", isDirectory: false)
            .path ?? "/tmp/remux.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("remux") && $0.hasSuffix(".sock") }
