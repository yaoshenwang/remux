...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuOpensKeyboardShortcutsSection() {
        ...
        app.launchEnvironment["REMUX_UI_TEST_MODE"] = "1"
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        let app = XCUIApplication()
        app.launchEnvironment["REMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_FEED_URL"] = "https://remux.test/appcast.xml"
        app.launchEnvironment["REMUX_UI_TEST_FEED_MODE"] = "available"
        app.launchEnvironment["REMUX_UI_TEST_UPDATE_VERSION"] = "9.9.9"
        app.launchEnvironment["REMUX_UI_TEST_AUTO_ALLOW_PERMISSION"] = "1"
        launchAndActivate(app)
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["REMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["REMUX_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let hiddenSurfaceToken = "remux-command-palette-hidden-surface"
    private let visibleSurfaceToken = "remux-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "remux-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/remux-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/remux-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "remux.settings" && data["settingsWindowIsKey"] == "1"
        }
