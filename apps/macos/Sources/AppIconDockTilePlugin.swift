...
private let remuxAppIconDidChangeNotification = Notification.Name("com.remux.appIconDidChange")
private let remuxAppIconModeKey = "appIconMode"
...
final class RemuxDockTilePlugin: NSObject, NSDockTilePlugIn {
    ...
    private let pluginBundle = Bundle(for: RemuxDockTilePlugin.self)
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        ...
        iconChangeObserver = DistributedNotificationCenter.default().addObserver(
            forName: remuxAppIconDidChangeNotification,
            object: nil,
            queue: nil
        ) { [weak self] _ in
    ...
    private func updateDockTile(_ dockTile: NSDockTile) {
        let mode = DockTileAppIconMode(defaultsValue: appDefaults?.string(forKey: remuxAppIconModeKey))
