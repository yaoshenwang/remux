...
#if os(macOS)
func remuxShouldApplyWindowGlass(
...
func remuxShouldUseTransparentBackgroundWindow() -> Bool {
    ...
    return remuxShouldApplyWindowGlass(
...
#if DEBUG
func remuxPasteboardStringContentsForTesting(_ pasteboard: NSPasteboard) -> String? {
    GhosttyPasteboardHelper.stringContents(from: pasteboard)
}

func remuxPasteboardImageFileURLForTesting(_ pasteboard: NSPasteboard) -> URL? {
    GhosttyPasteboardHelper.saveImageFileURLIfNeeded(from: pasteboard)
}

func remuxPasteboardImagePathForTesting(_ pasteboard: NSPasteboard) -> String? {
...
class GhosttyApp {
    ...
    private static let releaseBundleIdentifier = "com.remux.app"
                    ...
                    TerminalImageTransferPlanner.execute(
                        ...
                        uploadWorkspaceRemote: { fileURLs, operation, finish in
                            guard let workspace = MainActor.assumeIsolated({
                                ...
                                finish(.failure(NSError(domain: "remux.remote.paste", code: 3)))
    ...
    private func loadDefaultConfigFilesWithLegacyFallback(_ config: ghostty_config_t) {
        ...
        loadRemuxAppSupportGhosttyConfigIfNeeded(config)
        loadCJKFontFallbackIfNeeded(config)
        // remux provides the terminal background via backgroundView (CALayer)
    ...
    ///
    /// See: https://github.com/yaoshenwang/remux-macos/pull/1017
    private func loadCJKFontFallbackIfNeeded(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            lines,
            into: config,
            prefix: "remux-cjk-font-fallback",
    ...
    /// font already covers the ranges remux would otherwise auto-map.
    ...
    /// Returns only the CJK mappings remux should auto-inject after respecting
    ...
    /// Returns the top-level config paths that remux will actually load before
    ...
    static func loadedCJKScanPaths(
        ...
        let appSupportConfigURLs = remuxAppSupportConfigURLs(
    ...
    /// Scans a single config file for font settings relevant to remux's
    ...
    static func remuxAppSupportConfigURLs(
    ...
    private func loadRemuxAppSupportGhosttyConfigIfNeeded(_ config: ghostty_config_t) {
        #if os(macOS)
        ...
        let urls = Self.remuxAppSupportConfigURLs(
...
#if DEBUG
        dlog(
            "loaded remux app support ghostty config from: \(urls.map(\.path).joined(separator: ", "))"
        )
#endif
        #endif
    }
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    private let configTemplate: RemuxSurfaceConfigTemplate?
    ...
    func debugSurfaceContextLabel() -> String {
        remuxSurfaceContextName(surfaceContext)
    }
    ...
    func liveSurfaceForGhosttyAccess(reason: String) -> ghostty_surface_t? {
        ...
        guard registeredOwnerId == id,
              remuxSurfacePointerAppearsLive(surface) else {
    ...
    private func createSurface(for view: GhosttyNSView) {
        ...
        let baseConfig = configTemplate ?? RemuxSurfaceConfigTemplate()
        ...
        setManagedEnvironmentValue("REMUX_SURFACE_ID", id.uuidString)
        setManagedEnvironmentValue("REMUX_WORKSPACE_ID", tabId.uuidString)
        setManagedEnvironmentValue("REMUX_SURFACE_ID", id.uuidString)
        setManagedEnvironmentValue("REMUX_WORKSPACE_ID", tabId.uuidString)
        // Backward-compatible shell integration keys used by existing scripts/tests.
        setManagedEnvironmentValue("REMUX_PANEL_ID", id.uuidString)
        setManagedEnvironmentValue("REMUX_TAB_ID", tabId.uuidString)
        let socketPath = SocketControlSettings.socketPath()
        setManagedEnvironmentValue("REMUX_SOCKET_PATH", socketPath)
        setManagedEnvironmentValue("REMUX_SOCKET", socketPath)
        setManagedEnvironmentValue("REMUX_SOCKET_PATH", socketPath)
        // Backward-compatible alias expected by older scripts and third-party integrations.
        setManagedEnvironmentValue("REMUX_SOCKET", socketPath)
        let bundledCLIURL =
            Bundle.main.resourceURL?.appendingPathComponent("bin/remux")
            ?? Bundle.main.resourceURL?.appendingPathComponent("bin/remux")
        if let bundledCLIURL,
           FileManager.default.isExecutableFile(atPath: bundledCLIURL.path) {
            setManagedEnvironmentValue("REMUX_BUNDLED_CLI_PATH", bundledCLIURL.path)
            setManagedEnvironmentValue("REMUX_BUNDLED_CLI_PATH", bundledCLIURL.path)
        }
        if let bundleId = Bundle.main.bundleIdentifier, !bundleId.isEmpty {
            setManagedEnvironmentValue("REMUX_BUNDLE_ID", bundleId)
            setManagedEnvironmentValue("REMUX_BUNDLE_ID", bundleId)
        }
        ...
        do {
            ...
            setManagedEnvironmentValue("REMUX_PORT", String(startPort))
            setManagedEnvironmentValue("REMUX_PORT_END", String(startPort + Self.sessionPortRangeSize - 1))
            setManagedEnvironmentValue("REMUX_PORT_RANGE", String(Self.sessionPortRangeSize))
        }

        let claudeHooksEnabled = ClaudeCodeIntegrationSettings.hooksEnabled()
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("REMUX_CLAUDE_HOOKS_DISABLED", "1")
        }
        if let customClaudePath = ClaudeCodeIntegrationSettings.customClaudePath() {
            setManagedEnvironmentValue("REMUX_CUSTOM_CLAUDE_PATH", customClaudePath)
        }
        ...
        if shellIntegrationEnabled,
           let integrationDir = Bundle.main.resourceURL?.appendingPathComponent("shell-integration").path {
            setManagedEnvironmentValue("REMUX_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("REMUX_SHELL_INTEGRATION_DIR", integrationDir)
            ...
            if shellName == "zsh" {
                if GhosttyApp.shared.shellIntegrationMode() != "none" {
                    setManagedEnvironmentValue("REMUX_LOAD_GHOSTTY_ZSH_INTEGRATION", "1")
                }
                ...
                if let candidateZdotdir, !candidateZdotdir.isEmpty {
                    ...
                    if !isGhosttyInjected {
                        setManagedEnvironmentValue("REMUX_ZSH_ZDOTDIR", candidateZdotdir)
                    }
                }
                ...
                if GhosttyApp.shared.shellIntegrationMode() != "none" {
                    setManagedEnvironmentValue("REMUX_LOAD_GHOSTTY_BASH_INTEGRATION", "1")
                }
                // macOS ships /bin/bash 3.2, where Ghostty's automatic bash
                // integration is unsupported and HOME-based wrapper startup is
                // not reliable. Bootstrap remux bash integration on the first
                // interactive prompt instead.
                setManagedEnvironmentValue("PROMPT_COMMAND", """
                unset PROMPT_COMMAND; \
                if [[ "${REMUX_LOAD_GHOSTTY_BASH_INTEGRATION:-0}" == "1" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then \
                _remux_ghostty_bash="$GHOSTTY_RESOURCES_DIR/shell-integration/bash/ghostty.bash"; \
                [[ -r "$_remux_ghostty_bash" ]] && source "$_remux_ghostty_bash"; \
                fi; \
                if [[ "${REMUX_SHELL_INTEGRATION:-1}" != "0" && -n "${REMUX_SHELL_INTEGRATION_DIR:-}" ]]; then \
                _remux_bash_integration="$REMUX_SHELL_INTEGRATION_DIR/remux-bash-integration.bash"; \
                [[ -r "$_remux_bash_integration" ]] && source "$_remux_bash_integration"; \
                fi; \
                unset _remux_ghostty_bash _remux_bash_integration; \
                if declare -F _remux_prompt_command >/dev/null 2>&1; then _remux_prompt_command; fi
                """)
            }
        }
    ...
    static func handleDropForTesting(
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadDetectedSSH: { _, _, _, finish in
                finish(.failure(NSError(domain: "remux.remote.drop", code: 4)))
            },
            insertText: sendText,
        ...
        TerminalImageTransferPlanner.execute(
            ...
            uploadWorkspaceRemote: { [weak self] fileURLs, operation, finish in
                guard let workspace = MainActor.assumeIsolated({
                    ...
                    finish(.failure(NSError(domain: "remux.remote.drop", code: 3)))
                    GhosttyPasteboardHelper.cleanupTransferredTemporaryImageFiles(fileURLs)
                    return
                }
                workspace.uploadDroppedFilesForRemoteTerminal(
