...
enum SocketControlPasswordStore {
    ...
    private static let legacyKeychainService = "com.remux.app.socket-control"
    ...
    static func configuredPassword(
        ...
        if let envPassword = normalized(
            environment[SocketControlSettings.socketPasswordEnvKey]
                ?? environment[SocketControlSettings.legacySocketPasswordEnvKey]
        ) {
            return envPassword
        }
...
struct SocketControlSettings {
    ...
    private static let legacyAllowSocketPathOverrideKey = "REMUX_ALLOW_SOCKET_OVERRIDE"
    static let socketPasswordEnvKey = "REMUX_SOCKET_PASSWORD"
    static let legacySocketPasswordEnvKey = "REMUX_SOCKET_PASSWORD"
    static let launchTagEnvKey = "REMUX_TAG"
    private static let legacyLaunchTagEnvKey = "REMUX_TAG"
    static let baseDebugBundleIdentifier = "com.remux.macos.debug"
    private static let legacyBaseDebugBundleIdentifier = "com.remux.app.debug"
    private static let nightlyBundleIdentifier = "com.remux.macos.nightly"
    private static let legacyNightlyBundleIdentifier = "com.remux.app.nightly"
    private static let stagingBundleIdentifier = "com.remux.macos.staging"
    private static let legacyStagingBundleIdentifier = "com.remux.app.staging"
    private static let socketDirectoryName = "remux"
    private static let stableSocketFileName = "remux.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyStableDefaultSocketPath = "/tmp/remux.sock"
    static let legacyLastSocketPathFile = "/tmp/remux-last-socket-path"
    ...
    static func launchTag(
        ...
        guard let raw = environment[launchTagEnvKey] ?? environment[legacyLaunchTagEnvKey] else { return nil }
    ...
    static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any REMUX_UI_TEST_ or REMUX_UI_TEST_ env var.
        if environment.keys.contains(where: { $0.hasPrefix("REMUX_UI_TEST_") || $0.hasPrefix("REMUX_UI_TEST_") }) {
            return false
        }
    ...
    static func socketPath(
        ...
        if let taggedDebugPath = taggedDebugSocketPath(
            ...
            if isTruthy(environment[allowSocketPathOverrideKey]) || isTruthy(environment[legacyAllowSocketPathOverrideKey]),
               let override = environment["REMUX_SOCKET_PATH"] ?? environment["REMUX_SOCKET_PATH"],
               !override.isEmpty {
                return override
            }
        ...
        guard let override = environment["REMUX_SOCKET_PATH"] ?? environment["REMUX_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    static func defaultSocketPath(
        ...
        if bundleIdentifier == nightlyBundleIdentifier || bundleIdentifier == legacyNightlyBundleIdentifier {
    ...
    static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            .appendingPathComponent("remux-\(currentUserID).sock", isDirectory: false)
            .path ?? "/tmp/remux-\(currentUserID).sock"
    }
    ...
    static func recordLastSocketPath(_ path: String, filePath: String = lastSocketPathFile) {
        ...
        writeSocketPathMarker(payload, to: "/tmp/remux-last-socket-path")
    }
    ...
    /// Tagged DEV builds have bundle IDs like `com.remux.app.debug.<tag>`.
    ...
    static func taggedDebugSocketPath(
        ...
        let matchingBase: String?
        if bundleId.hasPrefix("\(baseDebugBundleIdentifier).") {
            matchingBase = baseDebugBundleIdentifier
        } else if bundleId.hasPrefix("\(legacyBaseDebugBundleIdentifier).") {
            matchingBase = legacyBaseDebugBundleIdentifier
        } else {
            matchingBase = nil
        }

        if let matchingBase {
            let suffix = String(bundleId.dropFirst(matchingBase.count + 1))
            let slug = suffix
                .replacingOccurrences(of: ".", with: "-")
                .trimmingCharacters(in: CharacterSet(charactersIn: "-"))
            if !slug.isEmpty {
                return "/tmp/remux-debug-\(slug).sock"
            }
        }
        ...
        return "/tmp/remux-debug-\(tag).sock"
    }
    ...
    static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == stagingBundleIdentifier
            ...
            || bundleIdentifier == legacyStagingBundleIdentifier
            || bundleIdentifier.hasPrefix("\(legacyStagingBundleIdentifier).")
    }
    ...
    static func envOverrideEnabled(
        ...
        guard let raw = environment["REMUX_SOCKET_ENABLE"] ?? environment["REMUX_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    static func envOverrideMode(
        ...
        guard let raw = environment["REMUX_SOCKET_MODE"] ?? environment["REMUX_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
