...
/// Manages trusted directories for remux.json command execution.
/// When a directory (or its git repo root) is trusted, `confirm: true` commands
/// from that directory's remux.json skip the confirmation dialog.
/// Global config (~/.config/remux/remux.json) is always trusted.
final class RemuxDirectoryTrust {
    static let shared = RemuxDirectoryTrust()
    ...
    private init() {
        ...
        ).first!.appendingPathComponent("remux")
    ...
    /// Check if a remux.json path is trusted.
    /// Global config is always trusted. For local configs, check the git repo root
    /// (or the remux.json parent directory if not in a git repo).
    ...
    /// Trust the directory containing a remux.json. If the remux.json is inside a git
    ...
    /// Resolve the trust key for a remux.json path: git repo root if inside a repo,
    /// otherwise the remux.json's parent directory.
