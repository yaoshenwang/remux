...
/// Hosts a popup `RemuxWebView` in a standalone `NSPanel`, created when a page
...
/// `remux_performKeyEquivalent` can dispatch it to the main menu's
...
final class BrowserPopupWindowController: NSObject, NSWindowDelegate {
    ...
    init(
        ...
        // overlaying the opener's browser context so OAuth popups keep remux's
        // shared cookie/storage scope and opener linkage.
        let webView = RemuxWebView(frame: .zero, configuration: configuration)
        ...
        panel.identifier = NSUserInterfaceItemIdentifier("remux.browser-popup")
