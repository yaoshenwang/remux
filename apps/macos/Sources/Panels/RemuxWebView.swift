...
extension WKWebView {
    var remuxIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func remuxIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard remuxIsElementFullscreenActiveOrTransitioning else { return false }
...
/// family before remux falls back to its own browser find overlay.
final class RemuxWebView: WKWebView {
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? RemuxWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: RemuxWebView) {
    ...
    var contextMenuLinkURLProvider: ((RemuxWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.remux.sidebar-tab-reorder"),
    ]
