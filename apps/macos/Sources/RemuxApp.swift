...
enum UITestLaunchManifest {
    static let argumentName = "-remuxUITestLaunchManifest"
...
struct RemuxApp: App {
    ...
    @StateObject private var remuxConfigStore = RemuxConfigStore()
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.remuxOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged remux DEV; start with ./scripts/reload.sh --tag <name> (or set REMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
            ...
            ContentView(updateViewModel: appDelegate.updateViewModel, windowId: primaryWindowId)
                ...
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["REMUX_UI_TEST_MODE"] == "1" {
                        UpdateLogStore.shared.append("ui test: remuxApp onAppear")
                    }
#endif
                    ...
                    applyAppearance()
                    if ProcessInfo.processInfo.environment["REMUX_UI_TEST_SHOW_SETTINGS"] == "1" {
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           remuxWindowShouldOwnCloseShortcut(window) {
...
private let remuxAuxiliaryWindowIdentifiers: Set<String> = [
    "remux.settings",
    "remux.about",
    "remux.licenses",
    "remux.browser-popup",
    "remux.settingsAboutTitlebarDebug",
    "remux.debugWindowControls",
    "remux.browserImportHintDebug",
    "remux.sidebarDebug",
    "remux.menubarDebug",
    "remux.backgroundDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func remuxWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return remuxAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("remux.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("remux.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, remux uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("remux.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("remux.settings.navigate")
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["REMUXCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["REMUX_COMMIT"] ?? ""
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.splitButtonLayoutDebug")
        window.center()
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("remux.backgroundDebug")
        window.center()
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.remux.appIconDidChange")
...
enum WelcomeSettings {
    static let shownKey = "remuxWelcomeShown"
}
...
struct SettingsView: View {
    ...
    @AppStorage("remuxPortBase") private var remuxPortBase = 9100
    @AppStorage("remuxPortRange") private var remuxPortRange = 10
    ...
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInRemuxBrowserKey) private var openTerminalLinksInRemuxBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInRemuxBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInRemuxBrowserKey)
    private var interceptTerminalOpenCommandInRemuxBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInRemuxBrowserValue()
    ...
    @AppStorage(BrowserLinkOpenSettings.openSidebarPortLinksInRemuxBrowserKey)
    private var openSidebarPortLinksInRemuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInRemuxBrowser
    ...
    @State private var trustedDirectoriesDraft: String = RemuxDirectoryTrust.shared.allTrustedPaths.joined(separator: "\n")
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When remux is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When remux is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
    ...
    private func saveTrustedDirectories() {
        let paths = trustedDirectoriesDraft
            ...
            .filter { !$0.isEmpty }
        RemuxDirectoryTrust.shared.replaceAll(with: paths)
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart remux to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep remux in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when remux highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $REMUX_NOTIFICATION_TITLE, $REMUX_NOTIFICATION_SUBTITLE, $REMUX_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve remux.")
                        ) {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in remux Browser"),
                            subtitle: openSidebarPullRequestLinksInRemuxBrowser
                                ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside remux browser.")
                                : String(localized: "settings.app.openSidebarPRLinks.subtitleOff", defaultValue: "Clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPullRequestLinksInRemuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                        ...
                        SettingsCardRow(
                            String(localized: "settings.app.openSidebarPortLinks", defaultValue: "Open Sidebar Port Links in remux Browser"),
                            subtitle: openSidebarPortLinksInRemuxBrowser
                                ? String(localized: "settings.app.openSidebarPortLinks.subtitleOn", defaultValue: "Port clicks open inside remux browser.")
                                : String(localized: "settings.app.openSidebarPortLinks.subtitleOff", defaultValue: "Port clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPortLinksInRemuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: REMUX_SOCKET_ENABLE, REMUX_SOCKET_MODE, and REMUX_SOCKET_PATH (set REMUX_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without remux integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, remux wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for REMUX_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $remuxPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $remuxPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets REMUX_PORT and REMUX_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        VStack(alignment: .leading, spacing: 6) {
                            SettingsCardRow(
                                ...
                                subtitle: String(localized: "settings.customCommands.trustedDirectories.subtitle", defaultValue: "Commands from remux.json in these directories run without confirmation. One path per line.")
                            ) {
                                EmptyView()
                            }
                        ...
                        SettingsCardDivider()
                        SettingsCardNote(String(localized: "settings.customCommands.trustedDirectories.note", defaultValue: "Place a remux.json in your project root to define custom commands. Trust a directory from the confirmation dialog, or add paths here. For git repos, trusting the root covers all subdirectories."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in remux Browser"),
                            ...
                            Toggle("", isOn: $openTerminalLinksInRemuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInRemuxBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInRemuxBrowser || interceptTerminalOpenCommandInRemuxBrowser {
                            SettingsCardDivider()

                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    String(localized: "settings.browser.hostWhitelist", defaultValue: "Hosts to Open in Embedded Browser"),
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in remux. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in remux.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in remux without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
    ...
    private func resetAllSettings() {
        ...
        openTerminalLinksInRemuxBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInRemuxBrowser
        interceptTerminalOpenCommandInRemuxBrowser = BrowserLinkOpenSettings.defaultInterceptTerminalOpenCommandInRemuxBrowser
        ...
        openSidebarPullRequestLinksInRemuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInRemuxBrowser
        openSidebarPortLinksInRemuxBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInRemuxBrowser
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("remux.") else { continue }
