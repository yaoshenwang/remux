...
func sidebarSelectedWorkspaceBackgroundNSColor(for colorScheme: ColorScheme) -> NSColor {
    ...
    return remuxAccentNSColor(for: colorScheme)
}
...
let commandPaletteOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("remux.yaoshen.wangmandPalette.overlay.container")
let tmuxWorkspacePaneOverlayContainerIdentifier = NSUserInterfaceItemIdentifier("remux.tmuxWorkspacePane.overlay.container")
...
struct ContentView: View {
    ...
    @EnvironmentObject var remuxConfigStore: RemuxConfigStore
    ...
    private var terminalContent: some View {
        ...
        return ZStack {
            // Show home panel when no workspace selected or Home is toggled
            if tabManager.isHomeSelected || (selectedWorkspaceId == nil && mountedWorkspaces.isEmpty),
               let server = AppDelegate.shared?.remuxServer {
                RemuxHomeView(serverProcess: server)
                    .transition(.opacity)
            }
            ZStack {
    ...
    var body: some View {
        ...
        view = AnyView(view.onAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak tabManager] in
                ...
                // Ensure there is at least one workspace (skip if Remux home is showing).
                if tabManager.tabs.isEmpty && !tabManager.isHomeSelected {
        ...
        view = AnyView(view.background(WindowAccessor { [sidebarBlendMode, bgGlassEnabled, bgGlassTintHex, bgGlassTintOpacity] window in
            ...
            let shouldApplyWindowGlass = remuxShouldApplyWindowGlass(
    ...
    private func commandPaletteCommandsFingerprint(commandsContext: CommandPaletteCommandsContext) -> Int {
        ...
        hasher.combine(remuxConfigStore.configRevision)
    ...
    private func commandPaletteCommandsContext(
        ...
        let cliInstalledInPATH = AppDelegate.shared?.isRemuxCLIInstalledInPATH() ?? false
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        let remuxConfigDefaultSubtitle = constant(String(localized: "command.remuxConfig.subtitle", defaultValue: "remux.json"))
        for command in remuxConfigStore.loadedCommands {
            let commandName = sanitizeRemuxConfigPaletteText(command.name)
            let subtitle = command.description
                .map { sanitizeRemuxConfigPaletteText($0) }
                .flatMap { $0.isEmpty ? nil : constant($0) }
                ?? remuxConfigDefaultSubtitle
            contributions.append(
                CommandPaletteCommandContribution(
                    commandId: command.id,
                    title: constant(String(localized: "command.remuxConfig.customTitle", defaultValue: "Custom: \(commandName)")),
                    subtitle: subtitle,
    ...
    private func sanitizeRemuxConfigPaletteText(_ text: String) -> String {
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        for command in remuxConfigStore.loadedCommands {
            ...
            let sourcePath = remuxConfigStore.commandSourcePaths[command.id]
            let globalPath = remuxConfigStore.globalConfigPath
    ...
    private func applyUITestSidebarSelectionIfNeeded(tabs: [Workspace]) {
        ...
        guard let rawValue = env["REMUX_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"]?
...
private struct SidebarTabItemSettingsSnapshot: Equatable {
    ...
    let showsSSH: Bool
    let openPullRequestLinksInRemuxBrowser: Bool
    let openPortLinksInRemuxBrowser: Bool
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        openPullRequestLinksInRemuxBrowser = BrowserLinkOpenSettings.openSidebarPullRequestLinksInRemuxBrowser(
            defaults: defaults
        )
        openPortLinksInRemuxBrowser = BrowserLinkOpenSettings.openSidebarPortLinksInRemuxBrowser(
            defaults: defaults
        )
            ...
            GeometryReader { proxy in
                ScrollView {
                    VStack(spacing: 0) {
                        ...
                        // Remux Home button
                        Button(action: { tabManager.isHomeSelected = true; tabManager.selectedTabId = nil }) {
                            HStack(spacing: 6) {
                            ...
                            .padding(.horizontal, 12)
                            .padding(.vertical, 6)
                            .background(tabManager.isHomeSelected ? Color.accentColor.opacity(0.2) : Color.clear)
...
private enum FeedbackComposerSettings {
    ...
    static let defaultEndpoint = "https://remux.yaoshen.wang/api/feedback"
...
private struct TabItemView: View, Equatable {
    ...
    private var openSidebarPullRequestLinksInRemuxBrowser: Bool {
        settings.openPullRequestLinksInRemuxBrowser
    }

    private var openSidebarPortLinksInRemuxBrowser: Bool {
        settings.openPortLinksInRemuxBrowser
    }
    ...
    private var activeUnreadBadgeFillColor: Color {
        ...
        return usesInvertedActiveForeground ? Color.white.opacity(0.25) : remuxAccentColor()
    }
    ...
    private func openPortLink(_ port: Int) {
        ...
        updateSelection()
        if openSidebarPortLinksInRemuxBrowser {
            if tabManager.openBrowser(
                inWorkspace: tab.id,
                url: url,
                preferSplitRight: true,
