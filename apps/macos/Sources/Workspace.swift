...
struct RemuxSurfaceConfigTemplate {
...
private func remuxPointerAppearsLive(_ pointer: UnsafeMutableRawPointer?) -> Bool {
    guard let pointer,
...
func remuxSurfacePointerAppearsLive(_ surface: ghostty_surface_t) -> Bool {
    ...
    remuxPointerAppearsLive(surface)
}

func remuxCurrentSurfaceFontSizePoints(_ surface: ghostty_surface_t) -> Float? {
    guard remuxSurfacePointerAppearsLive(surface) else {
        return nil
    }
...
) -> RemuxSurfaceConfigTemplate {
    let inherited = ghostty_surface_inherited_config(sourceSurface, context)
    var config = RemuxSurfaceConfigTemplate(cConfig: inherited)
...
// MARK: - remux.json custom layout

extension Workspace {

    func applyCustomLayout(_ layout: RemuxLayoutNode, baseCwd: String) {
        guard let rootPaneId = bonsplitController.allPaneIds.first else { return }

        var leaves: [(paneId: PaneID, surfaces: [RemuxSurfaceDefinition])] = []
    ...
    private func buildCustomLayoutTree(
        _ node: RemuxLayoutNode,
        inPane paneId: PaneID,
        leaves: inout [(paneId: PaneID, surfaces: [RemuxSurfaceDefinition])]
    ) {
        switch node {
        case .pane(let pane):
        ...
        case .split(let split):
            guard split.children.count == 2 else {
                NSLog("[RemuxConfig] split node requires exactly 2 children, got %d", split.children.count)
    ...
    private func populateCustomPane(
        _ paneId: PaneID,
        surfaces: [RemuxSurfaceDefinition],
        baseCwd: String,
    ...
    private func configureExistingSurface(
        panelId: UUID,
        inPane paneId: PaneID,
        surface: RemuxSurfaceDefinition,
        baseCwd: String,
        focusPanelId: inout UUID?
    ) {
        switch surface.type {
        case .terminal where surface.cwd != nil || surface.env != nil:
            ...
            let resolvedCwd = RemuxConfigStore.resolveCwd(surface.cwd, relativeTo: baseCwd)
    ...
    private func sendInputWhenReady(_ text: String, to panel: TerminalPanel) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            ...
            resolved = true
            if let observer { NotificationCenter.default.removeObserver(observer) }
            NSLog("[RemuxConfig] surface not ready after 3s, dropping command (%d chars)", text.count)
        }
    }
}
...
final class WorkspaceRemoteDaemonPendingCallRegistry {
    ...
    private let queue = DispatchQueue(label: "com.remux.remote-ssh.daemon-rpc.pending.\(UUID().uuidString)")
...
private final class WorkspaceRemoteDaemonRPCClient {
    ...
    private let writeQueue = DispatchQueue(label: "com.remux.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    private let stateQueue = DispatchQueue(label: "com.remux.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
    ...
    func start() throws {
        ...
        do {
            ...
            throw NSError(domain: "remux.remote.daemon.rpc", code: 1, userInfo: [
        ...
        do {
            ...
            guard capabilities.contains(Self.requiredProxyStreamCapability) else {
                throw NSError(domain: "remux.remote.daemon.rpc", code: 2, userInfo: [
    ...
    func openStream(host: String, port: Int, timeoutMs: Int = 10000) throws -> String {
        ...
        guard !streamID.isEmpty else {
            throw NSError(domain: "remux.remote.daemon.rpc", code: 3, userInfo: [
    ...
    func attachStream(
        ...
        guard !trimmedStreamID.isEmpty else {
            throw NSError(domain: "remux.remote.daemon.rpc", code: 17, userInfo: [
    ...
    private func call(method: String, params: [String: Any], timeout: TimeInterval) throws -> [String: Any] {
        ...
        do {
            ...
            throw NSError(domain: "remux.remote.daemon.rpc", code: 10, userInfo: [
        ...
        case .timedOut:
            ...
            throw NSError(domain: "remux.remote.daemon.rpc", code: 11, userInfo: [
                NSLocalizedDescriptionKey: "daemon RPC timeout waiting for \(method) response",
            ])
        case .failure(let failure):
            throw NSError(domain: "remux.remote.daemon.rpc", code: 12, userInfo: [
                NSLocalizedDescriptionKey: failure,
            ])
        case .missing:
            throw NSError(domain: "remux.remote.daemon.rpc", code: 13, userInfo: [
        ...
        throw NSError(domain: "remux.remote.daemon.rpc", code: 14, userInfo: [
    ...
    private func writePayload(_ payload: Data) throws {
        ...
        if stdinHandle === FileHandle.nullDevice {
            throw NSError(domain: "remux.remote.daemon.rpc", code: 15, userInfo: [
        ...
        do {
            ...
            stop(suppressTerminationCallback: false)
            throw NSError(domain: "remux.remote.daemon.rpc", code: 16, userInfo: [
...
private final class WorkspaceRemoteDaemonProxyTunnel {
    private final class ProxySession {
        ...
        private static let remoteLoopbackProxyAliasHost = "remux-loopback.localtest.me"
        ...
        private func parseSocksRequest(from data: Data) throws -> SocksRequest? {
            ...
            guard bytes[0] == 0x05 else {
                throw NSError(domain: "remux.remote.proxy", code: 1, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS version"])
            }
            ...
            default:
                throw NSError(domain: "remux.remote.proxy", code: 2, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS address type"])
            }

            guard !host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw NSError(domain: "remux.remote.proxy", code: 3, userInfo: [NSLocalizedDescriptionKey: "empty SOCKS host"])
            }
            ...
            guard port > 0 && port <= 65535 else {
                throw NSError(domain: "remux.remote.proxy", code: 4, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS port"])
            }
    ...
    private let queue = DispatchQueue(label: "com.remux.remote-ssh.daemon-tunnel.\(UUID().uuidString)", qos: .utility)
    ...
    func start() throws {
        ...
        queue.sync {
            guard !isStopped else {
                capturedError = NSError(domain: "remux.remote.proxy", code: 20, userInfo: [
    ...
    private static func makeLoopbackListener(port: Int) throws -> NWListener {
        guard let localPort = NWEndpoint.Port(rawValue: UInt16(port)) else {
            throw NSError(domain: "remux.remote.proxy", code: 21, userInfo: [
...
private final class WorkspaceRemoteProxyBroker {
    ...
    private let queue = DispatchQueue(label: "com.remux.remote-ssh.proxy-broker", qos: .utility)
...
private final class WorkspaceRemoteCLIRelayServer {
    private final class Session {
        ...
        private let challengeProtocol = "remux-relay-auth"
        ...
        private static func roundTripUnixSocket(socketPath: String, request: Data) throws -> Data {
            ...
            guard fd >= 0 else {
                throw NSError(domain: "remux.remote.relay", code: 1, userInfo: [
            ...
            guard pathBytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
                throw NSError(domain: "remux.remote.relay", code: 2, userInfo: [
            ...
            guard connectResult == 0 else {
                throw NSError(domain: "remux.remote.relay", code: 3, userInfo: [
                    NSLocalizedDescriptionKey: "failed to connect to local remux socket",
                ])
            }
            ...
            try request.withUnsafeBytes { rawBuffer in
                ...
                while bytesRemaining > 0 {
                    ...
                    if written <= 0 {
                        throw NSError(domain: "remux.remote.relay", code: 4, userInfo: [
            ...
            while true {
                ...
                if errno == EAGAIN || errno == EWOULDBLOCK {
                    ...
                    throw NSError(domain: "remux.remote.relay", code: 5, userInfo: [
                        NSLocalizedDescriptionKey: "timed out waiting for local remux response",
                    ])
                }
                throw NSError(domain: "remux.remote.relay", code: 6, userInfo: [
                    NSLocalizedDescriptionKey: "failed to read local remux response",
                ])
            }
            return response
        }
    ...
    private let relayID: String
    private let relayToken: Data
    private let queue = DispatchQueue(label: "com.remux.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
    ...
    init(localSocketPath: String, relayID: String, relayTokenHex: String) throws {
        guard let relayToken = Session.hexData(from: relayTokenHex), !relayToken.isEmpty else {
            throw NSError(domain: "remux.remote.relay", code: 7, userInfo: [
    ...
    func start() throws -> Int {
        ...
        if waitResult != .success {
            ...
            listener.stateUpdateHandler = nil
            listener.cancel()
            throw NSError(domain: "remux.remote.relay", code: 8, userInfo: [
...
final class WorkspaceRemoteSessionController {
    ...
    private let queue = DispatchQueue(label: "com.remux.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    private func beginConnectionAttemptLocked() {
        ...
        do {
            ...
            guard hello.capabilities.contains(WorkspaceRemoteDaemonRPCClient.requiredProxyStreamCapability) else {
                throw NSError(domain: "remux.remote.daemon", code: 43, userInfo: [
    ...
    private static let remotePlatformProbeOSMarker = "__REMUX_REMOTE_OS__="
    private static let remotePlatformProbeArchMarker = "__REMUX_REMOTE_ARCH__="
    private static let remotePlatformProbeExistsMarker = "__REMUX_REMOTE_EXISTS__="
    ...
    private func runProcess(
        ...
        let captureQueue = DispatchQueue(label: "remux.remote.process.capture")
        ...
        do {
            ...
            throw NSError(domain: "remux.remote.process", code: 1, userInfo: [
        ...
        if !didExitBeforeTimeout, process.isRunning {
            ...
            throw NSError(domain: "remux.remote.process", code: 2, userInfo: [
    ...
    private func installRemoteRelayMetadataLocked(
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "remux.remote.relay", code: 70, userInfo: [
    ...
    static func remoteRelayMetadataCleanupScript(relayPort: Int) -> String {
        ...
        socket_addr_file="$HOME/.remux/socket_addr"
        if [ -r "$socket_addr_file" ] && [ "$(tr -d '\\r\\n' < "$socket_addr_file")" = "$relay_socket" ]; then
          rm -f "$socket_addr_file"
        fi
        rm -f "$HOME/.remux/relay/\(relayPort).auth" "$HOME/.remux/relay/\(relayPort).daemon_path"
        """
    }

    private func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
        let script = """
        remux_uname_os="$(uname -s)"
        remux_uname_arch="$(uname -m)"
        printf '%s%s\\n' '\(Self.remotePlatformProbeOSMarker)' "$remux_uname_os"
        printf '%s%s\\n' '\(Self.remotePlatformProbeArchMarker)' "$remux_uname_arch"
        case "$(printf '%s' "$remux_uname_os" | tr '[:upper:]' '[:lower:]')" in
          linux|darwin|freebsd) remux_go_os="$(printf '%s' "$remux_uname_os" | tr '[:upper:]' '[:lower:]')" ;;
          *) exit 70 ;;
        esac
        case "$(printf '%s' "$remux_uname_arch" | tr '[:upper:]' '[:lower:]')" in
          x86_64|amd64) remux_go_arch=amd64 ;;
          aarch64|arm64) remux_go_arch=arm64 ;;
          armv7l) remux_go_arch=arm ;;
          *) exit 71 ;;
        esac
        remux_remote_path="$HOME/.remux/bin/remuxd-remote/\(version)/${remux_go_os}-${remux_go_arch}/remuxd-remote"
        if [ -x "$remux_remote_path" ]; then
        ...
        guard let unameOS, let unameArch else {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "remux.remote.daemon", code: 11, userInfo: [
        ...
        guard let goOS = Self.mapUnameOS(unameOS),
            ...
            throw NSError(domain: "remux.remote.daemon", code: 12, userInfo: [
        ...
        if result.status != 0, binaryExists == nil {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "remux.remote.daemon", code: 13, userInfo: [
    ...
    static let remoteDaemonManifestInfoKey = "REMUXRemoteDaemonManifestJSON"
    ...
    private static func allowLocalDaemonBuildFallback(environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool {
        environment["REMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1"
    }
    ...
    private static func explicitRemoteDaemonBinaryURL(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL? {
        ...
        guard let path = environment["REMUX_REMOTE_DAEMON_BINARY"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !path.isEmpty else {
            return nil
        }
    ...
    private static func versionedRemoteDaemonBuildURL(goOS: String, goArch: String, version: String) -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("remux-remote-daemon-build", isDirectory: true)
            .appendingPathComponent(version, isDirectory: true)
            .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
            .appendingPathComponent("remuxd-remote", isDirectory: false)
    }
    ...
    private static func fetchRemoteManifestLocked(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        guard let manifestURL = URL(string: "\(releaseURL)/remuxd-remote-manifest.json") else { return nil }
        let request = NSMutableURLRequest(url: manifestURL)
        request.timeoutInterval = 15
        request.setValue("remux/\(version)", forHTTPHeaderField: "User-Agent")
    ...
    private func downloadRemoteDaemonBinaryLocked(entry: WorkspaceRemoteDaemonManifest.Entry, version: String, releaseURL: String? = nil) throws -> URL {
        guard let url = URL(string: entry.downloadURL) else {
            throw NSError(domain: "remux.remote.daemon", code: 25, userInfo: [
        ...
        session.downloadTask(with: request as URLRequest) { localURL, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse,
               !(200...299).contains(httpResponse.statusCode) {
                downloadError = NSError(domain: "remux.remote.daemon", code: 26, userInfo: [
        ...
        guard let downloadedURL else {
            throw NSError(domain: "remux.remote.daemon", code: 27, userInfo: [
        ...
        if downloadedSHA != entry.sha256.lowercased() {
            ...
            if let releaseURL,
               ...
               downloadedSHA == liveEntry.sha256.lowercased() {
                ...
                throw NSError(domain: "remux.remote.daemon", code: 28, userInfo: [
    ...
    private func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
        ...
        guard Self.allowLocalDaemonBuildFallback() else {
            throw NSError(domain: "remux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "this build does not include a verified remuxd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set REMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback.",
            ])
        }

        guard let repoRoot = Self.findRepoRoot() else {
            throw NSError(domain: "remux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "cannot locate remux repo root for dev-only remuxd-remote build fallback",
            ])
        }
        ...
        guard FileManager.default.fileExists(atPath: goModPath) else {
            throw NSError(domain: "remux.remote.daemon", code: 21, userInfo: [
                NSLocalizedDescriptionKey: "missing daemon module at \(goModPath)",
            ])
        }
        guard let goBinary = Self.which("go") else {
            throw NSError(domain: "remux.remote.daemon", code: 22, userInfo: [
                NSLocalizedDescriptionKey: "go is required for the dev-only remuxd-remote build fallback",
            ])
        }
        ...
        let result = try runProcess(
            executable: goBinary,
            arguments: ["build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", output.path, "./cmd/remuxd-remote"],
            environment: env,
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "remux.remote.daemon", code: 23, userInfo: [
                NSLocalizedDescriptionKey: "failed to build remuxd-remote: \(detail)",
            ])
        }
        guard FileManager.default.isExecutableFile(atPath: output.path) else {
            throw NSError(domain: "remux.remote.daemon", code: 24, userInfo: [
                NSLocalizedDescriptionKey: "remuxd-remote build output is not executable",
            ])
        }
    ...
    private func uploadRemoteDaemonBinaryLocked(localBinary: URL, remotePath: String) throws {
        ...
        guard mkdirResult.status == 0 else {
            ...
            throw NSError(domain: "remux.remote.daemon", code: 30, userInfo: [
        ...
        guard scpResult.status == 0 else {
            ...
            throw NSError(domain: "remux.remote.daemon", code: 31, userInfo: [
                NSLocalizedDescriptionKey: "failed to upload remuxd-remote: \(detail)",
            ])
        }
        ...
        guard finalizeResult.status == 0 else {
            ...
            throw NSError(domain: "remux.remote.daemon", code: 32, userInfo: [
    ...
    static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/remux-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
    ...
    private func helloRemoteDaemonLocked(remotePath: String) throws -> DaemonHello {
        ...
        guard result.status == 0 else {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "remux.remote.daemon", code: 40, userInfo: [
        ...
        guard !responseLine.isEmpty,
            ...
            throw NSError(domain: "remux.remote.daemon", code: 41, userInfo: [
        ...
        if let ok = payload["ok"] as? Bool, !ok {
            ...
            throw NSError(domain: "remux.remote.daemon", code: 42, userInfo: [
        ...
        return DaemonHello(
            name: (name?.isEmpty == false ? name! : "remuxd-remote"),
    ...
    static func remoteCLIWrapperScript() -> String {
        ...
        daemon="$HOME/.remux/bin/remuxd-remote-current"
        socket_path="${REMUX_SOCKET_PATH:-}"
        if [ -z "$socket_path" ] && [ -r "$HOME/.remux/socket_addr" ]; then
          socket_path="$(tr -d '\\r\\n' < "$HOME/.remux/socket_addr")"
        fi

        if [ -n "$socket_path" ] && [ "${socket_path#/}" = "$socket_path" ] && [ "${socket_path#*:}" != "$socket_path" ]; then
          relay_port="${socket_path##*:}"
          relay_map="$HOME/.remux/relay/${relay_port}.daemon_path"
          if [ -r "$relay_map" ]; then
            mapped_daemon="$(tr -d '\\r\\n' < "$relay_map")"
    ...
    static func remoteCLIWrapperInstallScript(daemonRemotePath: String) -> String {
        ...
        return """
        mkdir -p "$HOME/.remux/bin" "$HOME/.remux/relay"
        ln -sf "$HOME/\(trimmedRemotePath)" "$HOME/.remux/bin/remuxd-remote-current"
        wrapper_tmp="$HOME/.remux/bin/.remux-wrapper.tmp.$$"
        cat > "$wrapper_tmp" <<'REMUXWRAPPER'
        \(remoteCLIWrapperScript())
        REMUXWRAPPER
        chmod 755 "$wrapper_tmp"
        mv -f "$wrapper_tmp" "$HOME/.remux/bin/remux"
        """
    }
    ...
    static func remoteRelayMetadataInstallScript(
        ...
        return """
        umask 077
        mkdir -p "$HOME/.remux" "$HOME/.remux/relay"
        chmod 700 "$HOME/.remux/relay"
        \(remoteCLIWrapperInstallScript(daemonRemotePath: trimmedRemotePath))
        printf '%s' "$HOME/\(trimmedRemotePath)" > "$HOME/.remux/relay/\(relayPort).daemon_path"
        cat > "$HOME/.remux/relay/\(relayPort).auth" <<'REMUXRELAYAUTH'
        \(authPayload)
        REMUXRELAYAUTH
        chmod 600 "$HOME/.remux/relay/\(relayPort).auth"
        printf '%s' '127.0.0.1:\(relayPort)' > "$HOME/.remux/socket_addr"
        """
    }
    ...
    private static func remoteDaemonPath(version: String, goOS: String, goArch: String) -> String {
        ".remux/bin/remuxd-remote/\(version)/\(goOS)-\(goArch)/remuxd-remote"
    }
    ...
    private static func findRepoRoot() -> URL? {
        ...
        if let envRoot = environment["REMUX_REMOTE_DAEMON_SOURCE_ROOT"],
        ...
        if let envRoot = environment["REMUXTERM_REPO_ROOT"],
           !envRoot.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            candidates.append(URL(fileURLWithPath: envRoot, isDirectory: true))
        }
...
final class Workspace: Identifiable, ObservableObject {
    enum RemuxWorkspaceKind: Equatable {
        case local
        case remote(url: String, label: String)
    }
    ...
    @Published var remuxKind: RemuxWorkspaceKind = .local
    ...
    private static let sshControlMasterCleanupQueue = DispatchQueue(
        label: "com.remux.remote-ssh.control-master-cleanup",
        qos: .utility
    )
    ...
    init(
        ...
        portOrdinal: Int = 0,
        configTemplate: RemuxSurfaceConfigTemplate? = nil,
    ...
    private func seedTerminalInheritanceFontPoints(
        panelId: UUID,
        configTemplate: RemuxSurfaceConfigTemplate?
    ) {
    ...
    private func resolvedTerminalInheritanceFontPoints(
        ...
        sourceSurface: ghostty_surface_t,
        inheritedConfig: RemuxSurfaceConfigTemplate
    ) -> Float? {
        let runtimePoints = remuxCurrentSurfaceFontSizePoints(sourceSurface)
    ...
    private func inheritedTerminalConfig(
        ...
        for terminalPanel in terminalPanelConfigInheritanceCandidates(
            ...
            // ghostty_surface_inherited_config or remuxCurrentSurfaceFontSizePoints
        ...
        if let fallbackFontPoints = lastTerminalConfigInheritanceFontPoints {
            var config = RemuxSurfaceConfigTemplate()
    ...
    private func promptMovePanel(tabId: TabID) {
        ...
        if !moved {
            ...
            failure.informativeText = String(localized: "alert.moveTab.failed.message", defaultValue: "remux could not move this tab to the selected destination.")
