...
struct GhosttyConfig {
    ...
    private static let remuxReleaseBundleIdentifier = "com.remux.app"
    ...
    private static func remuxConfigPaths(
        ...
        let releasePaths = paths(for: remuxReleaseBundleIdentifier)
        guard let currentBundleIdentifier, !currentBundleIdentifier.isEmpty else {
            return releasePaths
        }
        if currentBundleIdentifier == remuxReleaseBundleIdentifier {
            return releasePaths
        }
    ...
    private static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        ].map { NSString(string: $0).expandingTildeInPath } + remuxConfigPaths()
