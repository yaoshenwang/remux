...
extension Notification.Name {
    static let socketListenerDidStart = Notification.Name("remux.socketListenerDidStart")
    static let terminalSurfaceDidBecomeReady = Notification.Name("remux.terminalSurfaceDidBecomeReady")
    static let terminalSurfaceHostedViewDidMoveToWindow = Notification.Name("remux.terminalSurfaceHostedViewDidMoveToWindow")
    static let mainWindowContextsDidChange = Notification.Name("remux.mainWindowContextsDidChange")
    static let browserDownloadEventDidArrive = Notification.Name("remux.browserDownloadEventDidArrive")
}
...
class TerminalController {
    ...
    private var accessMode: SocketControlMode = .remuxOnly
    ...
    private static let v2BrowserEvalEnvelopeTypeKey = "__remux_t"
    private static let v2BrowserEvalEnvelopeValueKey = "__remux_v"
    ...
    private final class SocketFastPathState: @unchecked Sendable {
        private let queue = DispatchQueue(label: "com.remux.socket-fast-path")
    ...
    private func handleClient(_ socket: Int32, peerPid: pid_t? = nil) {
        ...
        // In remuxOnly mode, verify the connecting process is a descendant of remux.
        // In allowAll mode (env-var only), skip the ancestry check.
        if accessMode == .remuxOnly {
            ...
            if let pid {
                guard isDescendant(pid) else {
                    let msg = "ERROR: Access denied — only processes started inside remux can connect\n"
    ...
    private func processV2Command(_ jsonLine: String) -> String {
        ...
        // Remux
        case "remux.server.status":
            return v2Result(id: id, self.v2RemuxServerStatus(params: params))
        case "remux.server.start":
            return v2Result(id: id, self.v2RemuxServerStart(params: params))
        case "remux.server.stop":
            return v2Result(id: id, self.v2RemuxServerStop(params: params))
        case "remux.server.restart":
            return v2Result(id: id, self.v2RemuxServerRestart(params: params))
        case "remux.connect":
            return v2Result(id: id, self.v2RemuxConnect(params: params))
        case "remux.history.list":
            return v2Result(id: id, self.v2RemuxHistoryList(params: params))
        case "remux.history.clear":
            return v2Result(id: id, self.v2RemuxHistoryClear(params: params))
        case "remux.history.remove":
            return v2Result(id: id, self.v2RemuxHistoryRemove(params: params))
        case "remux.history.connect":
            return v2Result(id: id, self.v2RemuxHistoryConnect(params: params))
        case "remux.home":
            return v2Result(id: id, self.v2RemuxHome(params: params))
    ...
    private func v2Capabilities() -> [String: Any] {
        var methods: [String] = [
            ...
            "remux.server.start",
            "remux.server.stop",
            "remux.server.restart",
            "remux.connect",
            "remux.history.list",
            "remux.history.clear",
            "remux.history.remove",
            "remux.history.connect",
            "remux.home",
        ]
#if DEBUG
    ...
    // MARK: - V2 Remux Commands

    private func v2RemuxServerStatus(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            guard let server = AppDelegate.shared?.remuxServer else {
                return .err(code: "no_server", message: "Remux server manager not available", data: nil)
            }
            ...
            if let url = server.localURL { result["local_url"] = url }
            if let token = server.token { result["token"] = token }
            if let tunnel = server.tunnelURL { result["tunnel_url"] = tunnel }
            if let code = server.shortCode { result["short_code"] = code }
            return .ok(result)
        }
    }

    private func v2RemuxServerStart(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            guard let server = AppDelegate.shared?.remuxServer else {
                return .err(code: "no_server", message: "Remux server manager not available", data: nil)
            }
    ...
    private func v2RemuxServerStop(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            guard let server = AppDelegate.shared?.remuxServer else {
                return .err(code: "no_server", message: "Remux server manager not available", data: nil)
            }
    ...
    private func v2RemuxServerRestart(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            guard let server = AppDelegate.shared?.remuxServer else {
                return .err(code: "no_server", message: "Remux server manager not available", data: nil)
            }
    ...
    private func v2RemuxConnect(params: [String: Any]) -> V2CallResult {
        let target = (params["target"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !target.isEmpty else {
            return .err(code: "invalid_params", message: "target required (URL, ID code, or ws:// address)", data: nil)
        }

        let digits = target.filter(\.isNumber)
        let isShortCode = digits.count == 9 && target.filter(\.isLetter).isEmpty
        var resolvedURL = target
        var resolvedToken = (params["token"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""

        if isShortCode {
            guard let url = URL(string: "\(RemuxServerProcess.discoveryURL)/resolve/\(digits)") else {
                return .err(code: "invalid_url", message: "Invalid discovery URL", data: ["target": target])
            }
            let semaphore = DispatchSemaphore(value: 0)
            var resolution: (tunnelUrl: String, token: String?)?
            let task = URLSession.shared.dataTask(with: url) { data, response, _ in
                ...
                guard let data,
                      ...
                      let tunnelUrl = json["tunnelUrl"] as? String else {
                    return
                }
                resolution = (tunnelUrl, json["token"] as? String)
            }
            task.resume()
            _ = semaphore.wait(timeout: .now() + 10)
            task.cancel()
            guard let resolution else {
                return .err(code: "not_found", message: "Code not found or expired", data: ["target": target])
            }
            resolvedURL = resolution.tunnelUrl
                .replacingOccurrences(of: "https://", with: "wss://")
                .replacingOccurrences(of: "http://", with: "ws://")
            if resolvedToken.isEmpty, let token = resolution.token {
                resolvedToken = token
            }
        }

        return v2RemuxConnectResolved(
            url: resolvedURL,
            token: resolvedToken,
            labelOverride: params["label"] as? String,
            resolvedFromCode: isShortCode
        )
    }

    private func v2RemuxConnectResolved(
        url resolvedURL: String,
        token resolvedToken: String,
        labelOverride: String?,
        resolvedFromCode: Bool
    ) -> V2CallResult {
        return v2MainSync {
            guard let tabManager = self.tabManager else {
                return .err(code: "no_app", message: "App not ready", data: nil)
            }
            ...
            if let parsed = URL(string: url),
               let components = URLComponents(url: parsed, resolvingAgainstBaseURL: false) {
                if token.isEmpty,
                   let urlToken = components.queryItems?.first(where: { $0.name == "token" })?.value {
                    token = urlToken
                }
            ...
            guard !token.isEmpty else {
                return .err(code: "invalid_params", message: "token required", data: nil)
            }
            ...
            guard URL(string: url) != nil else {
                return .err(code: "invalid_url", message: "Invalid URL: \(url)", data: nil)
            }
            ...
            if let bundled = Bundle.main.resourceURL?.appendingPathComponent("bin/remux-bridge").path,
               FileManager.default.isExecutableFile(atPath: bundled) {
                ...
                guard let found = devPaths.first(where: { FileManager.default.isExecutableFile(atPath: $0) }) else {
                    return .err(code: "no_bridge", message: "remux-bridge executable not found", data: nil)
                }
                bridgePath = found
            }
            ...
            let host = URL(string: url)?.host ?? "Remote"
            let label = (labelOverride?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap {
                $0.isEmpty ? nil : $0
            } ?? host

            RemuxConnectionHistory.shared.add(url: url, token: token, label: label)

            let workspace = tabManager.addWorkspace(
                title: label,
                initialTerminalCommand: bridgePath,
                initialTerminalEnvironment: env,
                remuxKind: .remote(url: url, label: label)
            )
            return .ok([
                "workspace_id": workspace.id.uuidString,
                "label": label,
                "url": url,
                "resolved_from_code": resolvedFromCode,
            ])
        }
    }

    private func v2RemuxHistoryList(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            let entries = RemuxConnectionHistory.shared.entries.map { entry -> [String: Any] in
                [
                    "id": entry.id.uuidString,
                    "label": entry.label,
                    "url": entry.url,
                    "last_connected": ISO8601DateFormatter().string(from: entry.lastConnected),
                ]
            }
    ...
    private func v2RemuxHistoryClear(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            if let id = params["id"] as? String, let uuid = UUID(uuidString: id) {
                RemuxConnectionHistory.shared.remove(id: uuid)
            } else {
                RemuxConnectionHistory.shared.clearAll()
            }
            return .ok(["cleared": true])
        }
    }

    private func v2RemuxHistoryRemove(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            let query = (params["query"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            guard !query.isEmpty else {
                return .err(code: "invalid_params", message: "query required", data: nil)
            }
            guard let entry = RemuxConnectionHistory.shared.entry(matching: query) else {
                return .err(code: "not_found", message: "History entry not found", data: ["query": query])
            }
            RemuxConnectionHistory.shared.remove(id: entry.id)
            return .ok([
                "removed": true,
                "id": entry.id.uuidString,
                "label": entry.label,
    ...
    private func v2RemuxHistoryConnect(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            let query = (params["query"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            guard !query.isEmpty else {
                return .err(code: "invalid_params", message: "query required", data: nil)
            ...
            guard let token = RemuxConnectionHistory.shared.token(for: entry.id), !token.isEmpty else {
                return .err(code: "missing_token", message: "Saved token not found for history entry", data: [
                    "id": entry.id.uuidString,
                    "label": entry.label,
                    "url": entry.url,
                ])
            }
            return self.v2RemuxConnectResolved(
                url: entry.url,
                token: token,
    ...
    private func v2RemuxHome(params: [String: Any]) -> V2CallResult {
        return v2MainSync {
            guard let tabManager = self.tabManager else {
                return .err(code: "no_app", message: "App not ready", data: nil)
            }
            tabManager.isHomeSelected = true
            tabManager.selectedTabId = nil
            return .ok(["showing": "home"])
        }
    }
    ...
    private func v2RunBrowserJavaScript(
        ...
        if let frameSelector = v2BrowserCurrentFrameSelector(surfaceId: surfaceId) {
            ...
            framePrelude = """
            let __remuxDoc = document;
            try {
              const __remuxFrame = document.querySelector(\(selectorLiteral));
              if (__remuxFrame && __remuxFrame.contentDocument) {
                __remuxDoc = __remuxFrame.contentDocument;
              }
            } catch (_) {}
            """
        } else {
            framePrelude = "const __remuxDoc = document;"
        }
        ...
        const __remuxMaybeAwait = async (__r) => {
        ...
        const __remuxEvalInFrame = async function() {
          const document = __remuxDoc;
          \(executionBlock)
          const __value = await __remuxMaybeAwait(__r);
          return {
            __remux_t: (typeof __value === 'undefined') ? 'undefined' : 'value',
            __remux_v: __value
          };
        };

        return await __remuxEvalInFrame();
        """
    ...
    private func v2BrowserEnsureInitScriptsApplied(surfaceId: UUID, browserPanel: BrowserPanel) {
        ...
        let injector = """
        (() => {
          window.__remuxInitScriptsApplied = window.__remuxInitScriptsApplied || { scripts: [], styles: [] };
          return true;
        })()
        """
        ...
        for css in styles {
            ...
            let styleScript = """
            (() => {
              const id = 'remux-init-style-' + btoa(unescape(encodeURIComponent(\(cssLiteral)))).replace(/=+$/g, '');
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default.temporaryDirectory
                .appendingPathComponent("remux-browser-screenshots", isDirectory: true)
    ...
    private func v2BrowserFindWithScript(
        ...
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            \(finderBody)
              ...
              if (!__remuxFound) return { ok: false, error: 'not_found' };
              const selector = __remuxCssPath(__remuxFound);
              if (!selector) return { ok: false, error: 'not_found' };
              return {
                ok: true,
                selector,
                tag: String(__remuxFound.tagName || '').toLowerCase(),
                text: String(__remuxFound.textContent || '').trim()
              };
            })()
            """
    ...
    private func v2BrowserDialogRespond(params: [String: Any], accept: Bool) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const q = window.__remuxDialogQueue || [];
              ...
              if (entry.type === 'confirm') {
                ...
                window.__remuxDialogDefaults.confirm = \(acceptLiteral);
              }
              if (entry.type === 'prompt') {
                window.__remuxDialogDefaults = window.__remuxDialogDefaults || { confirm: false, prompt: null };
                if (\(acceptLiteral)) {
                  window.__remuxDialogDefaults.prompt = \(textLiteral);
                } else {
                  window.__remuxDialogDefaults.prompt = null;
                }
              }
    ...
    private func v2BrowserConsoleList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__remuxConsoleLog) ? window.__remuxConsoleLog.slice() : [];
              if (\(clearLiteral)) {
                window.__remuxConsoleLog = [];
              }
    ...
    private func v2BrowserErrorsList(params: [String: Any]) -> V2CallResult {
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let script = """
            (() => {
              const items = Array.isArray(window.__remuxErrorLog) ? window.__remuxErrorLog.slice() : [];
              if (\(clearLiteral)) {
                window.__remuxErrorLog = [];
              }
              return { ok: true, items };
            })()
            """
    ...
    private func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-screenshots")
