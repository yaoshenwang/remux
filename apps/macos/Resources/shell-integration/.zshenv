...

{
    ...
    if [[ -o interactive ]]; then
        ...
        if [[ "${REMUX_LOAD_GHOSTTY_ZSH_INTEGRATION:-0}" == "1" ]]; then
            if [[ -n "${REMUX_SHELL_INTEGRATION_DIR:-}" ]]; then
                builtin typeset _remux_ghostty="$REMUX_SHELL_INTEGRATION_DIR/ghostty-integration.zsh"
            fi
            if [[ ! -r "${_remux_ghostty:-}" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then
