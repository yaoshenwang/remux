...
xcodebuild -project GhosttyTabs.xcodeproj -scheme remux -configuration Release -destination 'platform=macOS' build
pkill -x remux || true
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/remux.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "remux.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/remux"
ATTEMPT=0
MAX_ATTEMPTS=20
