...
Builds remuxd-remote release assets for the supported remote platforms and emits:
  remuxd-remote-<goos>-<goarch>[-<suffix>]
  remuxd-remote-checksums[-<suffix>].txt
  remuxd-remote-manifest[-<suffix>].json
...
if ! command -v go >/dev/null 2>&1; then
  echo "error: go is required to build remuxd-remote release assets" >&2
  exit 1
fi
...
rm -f "$OUTPUT_DIR"/remuxd-remote-* "$OUTPUT_DIR"/remuxd-remote-checksums.txt "$OUTPUT_DIR"/remuxd-remote-manifest.json
...
CHECKSUMS_ASSET_NAME="remuxd-remote-checksums${SUFFIX_TAG}.txt"
CHECKSUMS_PATH="${OUTPUT_DIR}/${CHECKSUMS_ASSET_NAME}"
MANIFEST_PATH="${OUTPUT_DIR}/remuxd-remote-manifest${SUFFIX_TAG}.json"
...
: > "$CHECKSUMS_PATH"
ENTRIES_FILE="$(mktemp "${TMPDIR:-/tmp}/remuxd-remote-entries.XXXXXX")"
trap 'rm -f "$ENTRIES_FILE"' EXIT
: > "$ENTRIES_FILE"

for target in "${TARGETS[@]}"; do
  read -r GOOS GOARCH <<<"$target"
  ASSET_NAME="remuxd-remote-${GOOS}-${GOARCH}${SUFFIX_TAG}"
  ...
  BUILD_PATH="${OUTPUT_DIR}/remuxd-remote-${GOOS}-${GOARCH}.build"
  (
    cd "$DAEMON_ROOT"
    GOOS="$GOOS" \
    GOARCH="$GOARCH" \
...
echo "Built remuxd-remote assets in ${OUTPUT_DIR}"
