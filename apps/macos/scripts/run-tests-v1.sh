...
if [ "$(id -un)" != "remux" ]; then
  ...
  echo "Run via: ssh remux-vm 'cd /Users/remux/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/remux-tests-v1"
...
launch_and_wait() {
  ...
  defaults write com.remux.macos.debug socketControlMode -string full >/dev/null 2>&1 || true
