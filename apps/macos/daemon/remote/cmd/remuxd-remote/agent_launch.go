...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.REMUX_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
...
// runClaudeTeamsRelay implements `remux claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "remux claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		termEnvVar:     "REMUX_CLAUDE_TEAMS_TERM",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "remux claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "remux claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `remux omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "remux omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "remux omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "remux omo: plugin setup: %v\n", err)
		return 1
	}
	...
	fmt.Fprintf(os.Stderr, "remux omo: exec failed: %v\n", execErr)
	return 1
}
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".remuxterm", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "remux-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("REMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("REMUX_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("REMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("REMUX_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	os.Setenv(cfg.remuxBinEnvVar, selfPath)
	...
	// Socket path
	os.Setenv("REMUX_SOCKET_PATH", cfg.socketPath)
	os.Setenv("REMUX_SOCKET", cfg.socketPath)
	...
	if cfg.focused != nil {
		os.Setenv("REMUX_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("REMUX_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".remuxterm", "omo-config")
}
