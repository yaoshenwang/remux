...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "remuxd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  remuxd-remote version")
	_, _ = fmt.Fprintln(w, "  remuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  remuxd-remote cli <command> [args...]")
}
