...
func TestWrapperBinaryDispatchesIntoCLI(t *testing.T) {
	if os.Getenv("REMUXD_REMOTE_MAIN_HELPER") == "1" {
		separator := 0
	...
	wrapperPath := filepath.Join(t.TempDir(), "remuxd-remote-current")
	...
	cmd.Env = append(os.Environ(), "REMUXD_REMOTE_MAIN_HELPER=1")
