...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/remux-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"REMUX_CLAUDE_TEAMS_REMUX_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "REMUX_SOCKET_PATH", "REMUX_SOCKET", "TERM_PROGRAM",
		"REMUX_WORKSPACE_ID", "REMUX_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("REMUX_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("REMUX_SOCKET_PATH = %q", os.Getenv("REMUX_SOCKET_PATH"))
	}
	...
	if os.Getenv("REMUX_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("REMUX_WORKSPACE_ID = %q", os.Getenv("REMUX_WORKSPACE_ID"))
	}
	if os.Getenv("REMUX_SURFACE_ID") != "surf-789" {
		t.Errorf("REMUX_SURFACE_ID = %q", os.Getenv("REMUX_SURFACE_ID"))
	}
	// Verify extra env
