...
final class TabManagerPullRequestProbeTests: XCTestCase {
    func testGitHubRepositorySlugsPrioritizeUpstreamThenOriginAndDeduplicate() {
        let output = """
        origin https://github.com/austinwang/remux.git (fetch)
        origin https://github.com/austinwang/remux.git (push)
        upstream git@github.com:yaoshenwang/remux-macos.git (fetch)
        upstream git@github.com:yaoshenwang/remux-macos.git (push)
        backup ssh://git@github.com/yaoshenwang/remux-macos.git (fetch)
        mirror https://gitlab.com/yaoshenwang/remux-macos.git (fetch)
        """

        XCTAssertEqual(
            TabManager.githubRepositorySlugs(fromGitRemoteVOutput: output),
            ["yaoshenwang/remux-macos", "austinwang/remux"]
        )
    }
    ...
    func testPreferredPullRequestPrefersOpenOverMergedAndClosed() {
        let candidates = [
            TabManager.GitHubPullRequestProbeItem(
                number: 1889,
                state: "MERGED",
                url: "https://github.com/yaoshenwang/remux-macos/pull/1889",
                updatedAt: "2026-03-20T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1891,
                state: "OPEN",
                url: "https://github.com/yaoshenwang/remux-macos/pull/1891",
                updatedAt: "2026-03-19T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1800,
                state: "CLOSED",
                url: "https://github.com/yaoshenwang/remux-macos/pull/1800",
    ...
    func testPreferredPullRequestPrefersMostRecentlyUpdatedWithinSameStatus() {
        let olderOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1880,
            state: "OPEN",
            url: "https://github.com/yaoshenwang/remux-macos/pull/1880",
            updatedAt: "2026-03-18T18:00:00Z"
        )
        let newerOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1890,
            state: "OPEN",
            url: "https://github.com/yaoshenwang/remux-macos/pull/1890",
            updatedAt: "2026-03-20T18:00:00Z"
        )
    ...
    func testPreferredPullRequestIgnoresMalformedCandidates() {
        let valid = TabManager.GitHubPullRequestProbeItem(
            number: 1888,
            state: "OPEN",
            url: "https://github.com/yaoshenwang/remux-macos/pull/1888",
            updatedAt: "2026-03-20T18:00:00Z"
        )
        ...
        XCTAssertEqual(
            TabManager.preferredPullRequest(from: [
                TabManager.GitHubPullRequestProbeItem(
                    number: 9999,
                    state: "WHATEVER",
                    url: "https://github.com/yaoshenwang/remux-macos/pull/9999",
                    updatedAt: "2026-03-21T18:00:00Z"
                ),
                TabManager.GitHubPullRequestProbeItem(
                    number: 10000,
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/yaoshenwang/remux-macos/pull/371"))
    ...
    func testPeriodicWorkspaceGitMetadataRefreshUpdatesMainWorkspaceAfterCheckoutToFeatureBranch() throws {
        ...
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("remux-git-main-refresh-\(UUID().uuidString)")
        ...
        try runGit(["config", "user.name", "remux tests"], in: repoURL)
        try runGit(["config", "user.email", "remux@example.invalid"], in: repoURL)
        try "seed\n".write(
    ...
    func testResolvedCommandPathFallsBackOutsideAppPATH() throws {
        ...
        let tempDir = fileManager.temporaryDirectory.appendingPathComponent(
            "remux-command-path-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: tempDir) }

        let executableName = "remux-gh-test-\(UUID().uuidString)"
    ...
    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("remux-git-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/yaoshenwang/remux-macos/pull/1052")),
            status: .open,
