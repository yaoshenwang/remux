...
var remuxUnitTestInspectorAssociationKey: UInt8 = 0
var remuxUnitTestInspectorOverrideInstalled = false
var remuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = false
var remuxUnitTestWKWebViewPerformKeyEquivalentHook: ((WKWebView, NSEvent) -> Bool?)?

extension RemuxWebView {
...
extension WKWebView {
    @objc func remuxUnitTest_performKeyEquivalent(with event: NSEvent) -> Bool {
        if let hook = remuxUnitTestWKWebViewPerformKeyEquivalentHook,
           let result = hook(self, event) {
            return result
        }
        return remuxUnitTest_performKeyEquivalent(with: event)
    }
...
func installRemuxUnitTestInspectorOverride() {
...
func installRemuxUnitTestWKWebViewPerformKeyEquivalentOverride() {
    guard !remuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled else { return }

    let originalSelector = #selector(NSResponder.performKeyEquivalent(with:))
    let swizzledSelector = #selector(WKWebView.remuxUnitTest_performKeyEquivalent(with:))
    ...
    remuxUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = true
}
...
final class RemuxWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "RemuxWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
    ...
    func testCmdFDoesNotPreflightIntoPageWhenWebInspectorResponderIsFocused() {
        _ = NSApplication.shared
        installRemuxUnitTestWKWebViewPerformKeyEquivalentOverride()
        ...
        remuxUnitTestWKWebViewPerformKeyEquivalentHook = { currentWebView, event in
        ...
        window.makeKeyAndOrderFront(nil)
        defer {
            remuxUnitTestWKWebViewPerformKeyEquivalentHook = nil
            window.orderOut(nil)
        }
        ...
        XCTAssertEqual(
            ...
            0,
            "Did not expect RemuxWebView to preflight Cmd+F into page content while Web Inspector is focused"
        )
    }
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-browser-history-\(UUID().uuidString)", isDirectory: true)
