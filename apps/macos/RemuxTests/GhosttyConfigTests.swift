...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/remux-theme-a"
        let pathB = "/tmp/remux-theme-b"
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testRemuxAppSupportConfigURLsUseReleaseConfigForDebugBundleWithoutCurrentConfig() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            let releaseConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.remux.app",
                filename: "config",
                contents: "font-size = 13\n"
            )

            XCTAssertEqual(
                GhosttyApp.remuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.remux.app.debug",
                    appSupportDirectory: appSupportDirectory
                ),
                [releaseConfigURL]
            )
    ...
    func testRemuxAppSupportConfigURLsPreferCurrentBundleConfigWhenPresent() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            ...
            let currentConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.remux.app.debug.issue-829",
            ...
            XCTAssertEqual(
                GhosttyApp.remuxAppSupportConfigURLs(
                    currentBundleIdentifier: "com.remux.app.debug.issue-829",
                    appSupportDirectory: appSupportDirectory
                ),
                [currentConfigURL]
            )
    ...
    func testRemuxAppSupportConfigURLsSkipReleaseFallbackForNonDebugBundle() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.remux.app",
    ...
    func testRemuxAppSupportConfigURLsIgnoreMissingOrEmptyFiles() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.remux.app",
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "remux.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "remux.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    private func withTemporaryAppSupportDirectory(
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("remux-app-support-\(UUID().uuidString)", isDirectory: true)
...
final class WindowTransparencyDecisionTests: XCTestCase {
    ...
    func testTranslucentOpacityForcesClearWindowBackgroundOutsideSidebarBlendModePath() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertFalse(remuxShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(remuxShouldUseClearWindowBackground(for: 0.80))
            XCTAssertFalse(remuxShouldUseClearWindowBackground(for: 1.0))
        }
    }

    func testGlassEnabledDecisionIgnoresGlassImplementationAvailability() {
        XCTAssertTrue(
            remuxShouldApplyWindowGlass(
    ...
    func testBehindWindowGlassPathKeepsTransparentWindowEnabled() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertTrue(remuxShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(remuxShouldUseClearWindowBackground(for: 1.0))
        }
    }
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/yaoshenwang/remux-macos/releases/tag/v0.62.0",
          "checksumsAssetName": "remuxd-remote-checksums.txt",
          "checksumsURL": "https://github.com/yaoshenwang/remux-macos/releases/download/v0.62.0/remuxd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "remuxd-remote-linux-amd64",
              "downloadURL": "https://github.com/yaoshenwang/remux-macos/releases/download/v0.62.0/remuxd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "remuxd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/Application Support/remux/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "remuxd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: remux-loopback.localtest.me:3000\r\n" +
                "Origin: http://remux-loopback.localtest.me:3000\r\n" +
                "Referer: http://remux-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "remux-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("remux-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://remux-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: remux-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://remux-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://remux-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=remux-loopback.localtest.me; Path=/"))
    }
}

final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvREMUXKeys() {
        let replayPath = "/tmp/remux-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "REMUX_SURFACE_ID": "managed-surface"
            ],
            protectedKeys: ["PATH", "REMUX_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "REMUX_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        XCTAssertEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        XCTAssertEqual(merged["REMUX_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "REMUX_SURFACE_ID"],
            additionalEnvironment: [
                "REMUX_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1"
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "REMUX_SURFACE_ID": "override-surface"
            ]
        )

        XCTAssertEqual(merged["PATH"], "/usr/bin")
        XCTAssertEqual(merged["REMUX_SURFACE_ID"], "managed-surface")
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "remux-loopback.localtest.me")
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/remux-store-dest.sock",
                terminalStartupCommand: "ssh remux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/remux-store-source.sock",
                terminalStartupCommand: "ssh remux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/remux-test.sock",
            terminalStartupCommand: "ssh remux-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/remux-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/remux-a.sock",
            terminalStartupCommand: "ssh remux-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/remux-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/remux-b.sock",
            terminalStartupCommand: "ssh remux-macmini"
        )
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("remuxOnly"), .remuxOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.remuxOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["REMUX_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "REMUX_SOCKET_PATH": "/tmp/remux-debug-issue-153-tmux-compat.sock",
            ],
            bundleIdentifier: "com.remux.app",
            isDebugBuild: false,
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.remux.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/remux-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "REMUX_SOCKET_PATH": "/tmp/remux-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.remux.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/remux-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "REMUX_SOCKET_PATH": "/tmp/remux-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.remux.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/remux-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "REMUX_SOCKET_PATH": "/tmp/remux-debug-forced.sock",
        ...
        XCTAssertEqual(path, "/tmp/remux-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/remux-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.remux.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/remux-debug.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.remux.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/remux-staging.sock"
        )
    }
    ...
    func testUntaggedDebugBundleBlockedWithoutLaunchTag() {
        XCTAssertTrue(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.remux.app.debug",
                isDebugBuild: true
            )
        )
    }
    ...
    func testUntaggedDebugBundleAllowedWithLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["REMUX_TAG": "tests-v1"],
                bundleIdentifier: "com.remux.app.debug",
                isDebugBuild: true
            )
        )
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.remux.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives REMUX_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["REMUX_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.remux.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["remux", "-remuxUITestLaunchManifest", "/tmp/remux-ui-test-launch.json"]
            ),
            "/tmp/remux-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["remux", "-remuxUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"REMUX_TAG":"ui-tests-display","REMUX_SOCKET_PATH":"/tmp/remux-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["remux", UITestLaunchManifest.argumentName, "/tmp/remux-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["REMUX_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["REMUX_SOCKET_PATH"], "/tmp/remux-ui-tests.sock")
    }
}
...
final class PostHogAnalyticsPropertiesTests: XCTestCase {
    ...
    func testSuperPropertiesIncludePlatformVersionAndBuild() {
        ...
        XCTAssertEqual(properties["platform"] as? String, "remux")
        XCTAssertEqual(properties["app_version"] as? String, "0.31.0")
        XCTAssertEqual(properties["app_build"] as? String, "230")
    }
    ...
    func testPropertiesOmitVersionFieldsWhenUnavailable() {
        ...
        XCTAssertEqual(superProperties["platform"] as? String, "remux")
    ...
    func testFlushPolicyIncludesDailyAndHourlyActiveEvents() {
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "remux_daily_active"))
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "remux_hourly_active"))
        XCTAssertFalse(PostHogAnalytics.shouldFlushAfterCapture(event: "remux_other_event"))
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "remux-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsSkipsReleaseAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("remux-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.remux-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.remux-dev",
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenRemuxRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(remuxLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutRemuxHandoffFlag() throws {
        let output = try runInteractiveZsh(remuxLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            remuxLoadGhosttyIntegration: true,
            remuxLoadShellIntegration: true,
            command: """
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            remuxLoadGhosttyIntegration: false,
            remuxLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedRemuxEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("remux-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            remuxLoadGhosttyIntegration: false,
            remuxLoadShellIntegration: true,
            command: "_remux_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "REMUX_SOCKET_PATH": "/tmp/remux-current.sock",
                "REMUX_TAG": "feat-tmux-notification-attention-state",
                "REMUX_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "REMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "REMUX_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "REMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g REMUX_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g REMUX_SOCKET_PATH /tmp/remux-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g REMUX_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g REMUX_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g REMUX_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("remux-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'REMUX_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'REMUX_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu REMUX_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu REMUX_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedRemuxEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("remux-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            remuxLoadGhosttyIntegration: false,
            remuxLoadShellIntegration: true,
            command: "_remux_precmd; print -r -- \"$REMUX_TAG|$REMUX_SOCKET_PATH|$REMUX_WORKSPACE_ID|$REMUX_SURFACE_ID|$REMUX_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "REMUX_SOCKET_PATH": "/tmp/remux-stale.sock",
                "REMUX_TAG": "feat-tmux-integration-experiments",
                "REMUX_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "REMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "REMUX_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "REMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/remux-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            remuxLoadShellIntegration: true,
            command: """
            _REMUX_TTY_NAME=ttys999
            print -r -- "$(_remux_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    private func runInteractiveZsh(remuxLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            remuxLoadGhosttyIntegration: remuxLoadGhosttyIntegration,
            remuxLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        remuxLoadGhosttyIntegration: Bool,
        remuxLoadShellIntegration: Bool,
        command: String,
        extraEnvironment: [String: String] = [:]
    ) throws -> String {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("remux-zsh-shell-integration-\(UUID().uuidString)")
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let remuxZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": remuxZdotdir.path,
            "REMUX_ZSH_ZDOTDIR": userZdotdir.path,
            "REMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if remuxLoadGhosttyIntegration {
            process.environment?["REMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if remuxLoadShellIntegration {
            process.environment?["REMUX_SHELL_INTEGRATION"] = "1"
            process.environment?["REMUX_SHELL_INTEGRATION_DIR"] = remuxZdotdir.path
            process.environment?["REMUX_SOCKET_PATH"] = root.appendingPathComponent("remux-test.sock").path
            process.environment?["REMUX_TAB_ID"] = "tab-test"
            process.environment?["REMUX_PANEL_ID"] = "panel-test"
        }
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("remux-browser-detect-\(UUID().uuidString)")
    }
