...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("remux-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".remux/relay/64011.shell")
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("remux-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".remux/relay")
        let socketAddrURL = home.appendingPathComponent(".remux/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("remux-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".remux/relay")
        let socketAddrURL = home.appendingPathComponent(".remux/socket_addr")
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/remux-debug-test.sock",
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to remux-macmini",
            target: "remux-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "remux-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/remux-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "remux-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/remux-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/remux-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/remux-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/remux-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to remux-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "remux-macmini")
        ...
        XCTAssertEqual(
            ...
            "Remote proxy unavailable (remux-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to remux-macmini", target: "remux-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to remux-macmini via shared local proxy 127.0.0.1:9999",
            target: "remux-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "remux",
                  item.path.contains(".app/Contents/Resources/bin/remux") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled remux CLI not found in \(appBundleURL.path)")
    }
    ...
    func testNotifyFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        environment["REMUX_SOCKET_PATH"] = socketPath
        environment["REMUX_WORKSPACE_ID"] = staleWorkspace
        environment["REMUX_SURFACE_ID"] = staleSurface
        environment["REMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["REMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "--ssh-option", "ControlPath=/tmp/remux-ssh-%C",
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=remux-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "remux-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath=/tmp/remux-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `remux ssh` should land the user in the new SSH workspace immediately.
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["REMUX_WORKSPACE_ID"] = staleWorkspace
        environment["REMUX_SURFACE_ID"] = staleSurface
        environment["REMUX_CLI_TTY_NAME"] = callerTTY
        environment["REMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["REMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyInTmuxPrefersCallerTTYOverStaleValidSurfaceID() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["REMUX_SOCKET_PATH"] = socketPath
        environment["REMUX_WORKSPACE_ID"] = workspaceId
        environment["REMUX_SURFACE_ID"] = staleSurface
        environment["REMUX_CLI_TTY_NAME"] = callerTTY
