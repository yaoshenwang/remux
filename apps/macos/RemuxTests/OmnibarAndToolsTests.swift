...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "remux-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../remux.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/remux.app/Contents/MacOS/remux", isDirectory: false),
            URL(fileURLWithPath: "/Applications/remux.app/Contents/Resources/bin/remux", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/remux", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/remux/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/remux",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/remux.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/remux.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/remux.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/remux.app backup/project",
                "/Applications/remux.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("remux.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/remux", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("remux.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("remux.app/Contents/MacOS/remux", isDirectory: false),
                    aliasWorkspace,
                ],
