...
private extension GhosttyNSView {
    @objc func remuxUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        remuxUnitTest_interpretKeyEvents(eventArray)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.remuxUnitTest_interpretKeyEvents(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in remux)
