...
final class RemuxConfigDecodingTests: XCTestCase {

    private func decode(_ json: String) throws -> RemuxConfigFile {
...
final class RemuxCommandIdentityTests: XCTestCase {

    func testCommandIdIsDeterministic() {
        let cmd = RemuxCommandDefinition(name: "Run tests", command: "test")
        XCTAssertEqual(cmd.id, "remux.config.command.Run%20tests")
    }

    func testCommandIdEncodesSpecialCharacters() {
        let cmd = RemuxCommandDefinition(name: "build & deploy", command: "make")
        XCTAssertTrue(cmd.id.hasPrefix("remux.config.command."))
        XCTAssertFalse(cmd.id.contains("&"))
        XCTAssertFalse(cmd.id.contains(" "))
    }

    func testCommandIdIsUniqueForDifferentNames() {
        let cmd1 = RemuxCommandDefinition(name: "build", command: "make build")
        let cmd2 = RemuxCommandDefinition(name: "test", command: "make test")
        XCTAssertNotEqual(cmd1.id, cmd2.id)
    }

    func testCommandIdDoesNotCollideWithBuiltinPrefix() {
        let cmd = RemuxCommandDefinition(name: "palette.newWorkspace", command: "echo")
        XCTAssertTrue(cmd.id.hasPrefix("remux.config.command."))
...
final class RemuxSplitDefinitionTests: XCTestCase {
    ...
    func testSplitOrientationVertical() {
        let split = RemuxSplitDefinition(direction: .vertical, split: nil, children: [])
...
@MainActor
final class RemuxConfigCwdResolutionTests: XCTestCase {

    private let baseCwd = "/Users/test/project"

    func testNilCwdReturnsBase() {
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd(nil, relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testEmptyCwdReturnsBase() {
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd("", relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testAbsolutePathReturnedAsIs() {
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd("/tmp/other", relativeTo: baseCwd),
            "/tmp/other"
        )
    }
    ...
    func testRelativePathJoinedToBase() {
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd("backend/src", relativeTo: baseCwd),
    ...
    func testTildeSlashExpandsToHomePlusPath() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd("~/Documents/work", relativeTo: baseCwd),
    ...
    func testSingleSubdirectory() {
        XCTAssertEqual(
            RemuxConfigStore.resolveCwd("src", relativeTo: baseCwd),
...
final class RemuxLayoutEncodingTests: XCTestCase {

    func testPaneNodeRoundTrips() throws {
        let original = RemuxLayoutNode.pane(RemuxPaneDefinition(surfaces: [
            RemuxSurfaceDefinition(type: .terminal, name: "shell")
        ]))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RemuxLayoutNode.self, from: data)
    ...
    func testSplitNodeRoundTrips() throws {
        let original = RemuxLayoutNode.split(RemuxSplitDefinition(
            direction: .vertical,
            split: 0.7,
            children: [
                .pane(RemuxPaneDefinition(surfaces: [RemuxSurfaceDefinition(type: .terminal)])),
                .pane(RemuxPaneDefinition(surfaces: [RemuxSurfaceDefinition(type: .browser, url: "http://localhost")]))
            ]
        ))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(RemuxLayoutNode.self, from: data)
