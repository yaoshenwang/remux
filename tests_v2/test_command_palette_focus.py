...
def _focused_surface_id(client: remux) -> str:
...
def main() -> int:
    token = "REMUX_PALETTE_FOCUS_PROBE_9412"
    restore_token = "REMUX_PALETTE_RESTORE_PROBE_7731"
    ...
    with remux(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise remuxError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise remuxError("terminal did not receive typing after closing command palette")
