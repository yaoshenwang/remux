#!/usr/bin/env python3
"""
Regression test: `remux --version` should print version text without requiring a socket.
"""
...
def main() -> int:
    ...
    if not out:
        print("FAIL: `remux --version` produced empty stdout")
        return 1
    ...
    if out2 != out:
        print("FAIL: `remux --version` and `remux version` differ")
        print(f"--version: {out!r}")
        print(f"version:   {out2!r}")
        return 1

    print(f"PASS: remux version command works ({out})")
    return 0
