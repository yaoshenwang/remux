...
In remuxOnly mode (default), only processes descended from the remux
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: remuxOnly — external processes get rejected
  Phase 2: remuxOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("REMUX_TAG") or os.environ.get("REMUX_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/remux DEV.app")
...
def _find_app():
    explicit = os.environ.get("REMUX_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/remux-{preferred_slug}/Build/Products/Debug/remux DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/remux-{preferred_slug}/Build/Products/Debug/remux DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/remux DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/remux-*/Build/Products/Debug/remux DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/remux-*/Build/Products/Debug/remux DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "remux")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/remux"
    )))
    candidates.extend(glob.glob("/tmp/remux-*/Build/Products/Debug/remux"))
    candidates.extend(glob.glob("/private/tmp/remux-*/Build/Products/Debug/remux"))
...
def _kill_remux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/remux DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "remux DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_remux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"REMUX_SOCKET_MODE={mode}"]
    launch_env = {
        "REMUX_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a remux-spawned terminal process CAN connect in remuxOnly mode.
    Inject a test via the shell rc file, then launch remux in remuxOnly mode.
    The shell (a descendant of remux) runs the test on startup.
    """
    result = TestResult("Internal process can connect (remuxOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"remux_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"remux_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing remux, launch in remuxOnly mode (default)
        _kill_remux(app_path)
        _launch_remux(app_path, socket_path, mode="remuxOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in remuxOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify REMUX_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_remux(app_path)
        _launch_remux(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"remux-pass-{os.getpid()}"
    try:
        _kill_remux(app_path)
        _launch_remux(
            app_path,
            socket_path,
            mode="password",
            extra_env={"REMUX_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find remux CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("remux Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find remux DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/remux-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: remuxOnly — external rejection ──
    print("Phase 1: remuxOnly mode — external rejection")
    print("-" * 50)

    # Ensure remux is running in remuxOnly mode
    _kill_remux(app_path)
    print("  Launching remux in remuxOnly mode...")
    _launch_remux(app_path, socket_path, mode="remuxOnly")
    ...
    # ── Phase 2: remuxOnly — internal process CAN connect ──
    print("Phase 2: remuxOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave remux in remuxOnly mode ──
    _kill_remux(app_path)
    _launch_remux(app_path, socket_path, mode="remuxOnly")
