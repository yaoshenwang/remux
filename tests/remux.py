...
A client library for programmatically controlling remux via Unix socket.
...
_LAST_SOCKET_PATH_FILE = "/tmp/remux-last-socket-path"
_DEFAULT_DEBUG_BUNDLE_ID = "com.remuxterm.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("REMUX_BUNDLE_ID")
    if override:
        return override

    tag = os.environ.get("REMUX_TAG")
    if tag:
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/remux-debug-{slug}.sock",
            f"/tmp/remux-{slug}.sock",
        ]
    ...
    override = os.environ.get("REMUX_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/remux-debug-*.sock")
...
class remux:
    """Client for controlling remux via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise remuxError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise remuxError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise remuxError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise remuxError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise remuxError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise remuxError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise remuxError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for remux"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="remux CLI")
    ...
    try:
        with remux(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("remux CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
