...
def wait_for_notifications(client: remux, expected: int, timeout: float = 2.0) -> list[dict]:
    start = time.time()
...
def wait_for_flash_count(client: remux, surface: str, minimum: int = 1, timeout: float = 2.0) -> int:
...
def ensure_two_surfaces(client: remux) -> list[tuple[int, str, bool]]:
...
def focused_surface_index(client: remux) -> int:
    surfaces = client.list_surfaces()
...
def send_osc(client: remux, sequence: str, surface: Optional[int] = None) -> None:
...
def test_clear_prior_notifications(client: remux) -> TestResult:
...
def test_suppress_when_focused(client: remux) -> TestResult:
...
def test_not_suppressed_when_inactive(client: remux) -> TestResult:
...
def test_kitty_notification_simple(client: remux) -> TestResult:
...
def test_kitty_notification_chunked(client: remux) -> TestResult:
...
def test_rxvt_notification_osc777(client: remux) -> TestResult:
...
def test_mark_read_on_focus_change(client: remux) -> TestResult:
...
def test_mark_read_on_app_active(client: remux) -> TestResult:
...
def test_mark_read_on_tab_switch(client: remux) -> TestResult:
...
def test_flash_on_tab_switch(client: remux) -> TestResult:
...
def test_focus_on_notification_click(client: remux) -> TestResult:
...
def test_restore_focus_on_tab_switch(client: remux) -> TestResult:
...
def test_clear_on_tab_close(client: remux) -> TestResult:
