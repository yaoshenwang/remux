...
remux uses isolated browser contexts per surface. Treat each browser surface as its own session.
...
# session B
remux browser open https://example.com --json
# -> surface:8

remux browser surface:7 get url
remux browser surface:8 get url
```
...
```bash
remux browser surface:7 state save /tmp/auth-state.json
```

### Load State

```bash
remux browser surface:8 state load /tmp/auth-state.json
...
remux browser open https://app.example.com/login --json
# login on surface:7 ...
remux browser surface:7 state save /tmp/auth.json

remux browser open https://app.example.com --json
# assume surface:8
remux browser surface:8 state load /tmp/auth.json
remux browser surface:8 goto https://app.example.com/dashboard
```
...
```bash
remux browser open https://site-a.example --json
remux browser open https://site-b.example --json
remux browser open https://site-c.example --json

remux browser surface:11 get text body > /tmp/a.txt
remux browser surface:12 get text body > /tmp/b.txt
remux browser surface:13 get text body > /tmp/c.txt
```
...
remux close-surface --surface surface:8
