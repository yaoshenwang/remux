...
# Absolute path
remux markdown open /path/to/PLAN.md

# Target a specific workspace
remux markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in remux:
...
```bash
# Open in the caller's workspace (default -- uses REMUX_WORKSPACE_ID)
remux markdown open plan.md

# Open in a specific workspace
remux markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
remux markdown open plan.md --surface surface:5

# Open in a specific window
remux markdown open plan.md --window window:1
```
