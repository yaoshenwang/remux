...
private final class CLISocketSentryTelemetry {
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["REMUX_SOCKET_PATH"] ?? processEnv["REMUX_SOCKET"]
        self.workspaceId = processEnv["REMUX_WORKSPACE_ID"]
        self.surfaceId = processEnv["REMUX_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["REMUX_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["REMUX_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        let tmpSockets = Self.discoverTmpRemuxSockets(limit: 10)
        if !tmpSockets.isEmpty {
            context["tmp_remux_sockets"] = tmpSockets
        }
        let taggedSockets = tmpSockets.filter { $0 != "/tmp/remux.sock" }
        if socketPath == "/tmp/remux.sock",
    ...
    private static func discoverTmpRemuxSockets(limit: Int) -> [String] {
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.remuxterm/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["REMUX_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
private enum SocketPasswordResolver {
    ...
    static func resolve(explicit: String?) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["REMUX_SOCKET_PASSWORD"]) {
            return env
        }
        return loadFromFile()
    }
...
private enum CLISocketPathResolver {
    ...
    private static func candidatePaths(requestedPath: String, environment: [String: String]) -> [String] {
        ...
        if let tag = normalized(environment["REMUX_TAG"]) {
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for name in entries where name.hasPrefix("remux") && name.hasSuffix(".sock") {
            let path = "/tmp/\(name)"
            var st = stat()
...
struct REMUXCLI {
    ...
    func run() throws {
        ...
        // so help text is available even when remux is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'remux help' to see available commands.")
            return
        }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let workspaceArg = tfWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (tfWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
        ...
        case "notify":
            ...
            let workspaceArg = notifyWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (notifyWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"],
        ...
        case "set-status":
            ...
            let workspaceArg = wsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let wsId = try resolveWorkspaceId(workspaceArg, client: client)
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
                )
        ...
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: remux markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    private func openPath(_ path: String, socketPath: String) throws {
        ...
        if (try? client.connect()) == nil {
            ...
            guard connected else {
                throw CLIError(message: "remux app did not start in time (socket not found at \(socketPath))")
            }
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["REMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["REMUX_TAB_ID"] ?? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"])
                : nil)
    ...
    private func runBrowserCommand(
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["REMUX_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "rename-tab":
            ...
            2) $REMUX_TAB_ID / $REMUX_SURFACE_ID
        ...
        case "new-workspace":
            return """
            Usage: remux new-workspace [--cwd <path>] [--command <text>]
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["REMUX_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["REMUX_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"])
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        ...
        for entry in entries where !entry.isEmpty {
            ...
            guard !isRemuxClaudeWrapper(at: candidate) else { continue }
            return candidate
        }
        return nil
    }
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        let fakeTmuxValue: String = {
            if let focusedContext {
                ...
                return "/tmp/remux-claude-teams/\(focusedContext.workspaceId),\(windowToken),\(focusedContext.paneHandle)"
            }
            return processEnvironment["TMUX"] ?? "/tmp/remux-claude-teams/default,0,0"
        }()
        let fakeTmuxPane = focusedContext.map { "%\($0.paneHandle)" }
            ?? processEnvironment["TMUX_PANE"]
            ?? "%1"
        let fakeTerm = processEnvironment["REMUX_CLAUDE_TEAMS_TERM"] ?? "screen-256color"

        setenv("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", "1", 1)
        setenv("REMUX_CLAUDE_TEAMS_REMUX_BIN", executablePath, 1)
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        let rootPath = URL(fileURLWithPath: homePath, isDirectory: true)
            .appendingPathComponent(".remuxterm", isDirectory: true)
    ...
    private func tmuxCompatStoreURL() -> URL {
        let root = NSString(string: "~/.remuxterm").expandingTildeInPath
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["REMUX_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["REMUX_SURFACE_ID"] : nil)
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                remux claude-hook <session-start|stop|notification> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func printWelcome() {
        ...
        print(shortcuts)
        print()
        print("  \(bold)Docs\(reset)\(dim)                https://remux.dev/docs\(reset)")
        print("  \(bold)Discord\(reset)\(dim)             https://discord.gg/xsgFEVrWCZ\(reset)")
        print("  \(bold)GitHub\(reset)\(dim)              https://github.com/yaoshenwang/remux (please leave a star ⭐)\(reset)")
        print("  \(bold)Email\(reset)\(dim)               founders@manaflow.com\(reset)")
        print()
        print("  \(dim)Run \(reset)\(bold)remux --help\(reset)\(dim) for all commands.\(reset)")
        print("  \(dim)Run \(reset)\(bold)remux shortcuts\(reset)\(dim) to edit shortcuts.\(reset)")
        print("  \(dim)Run \(reset)\(bold)remux feedback\(reset)\(dim) to report a bug.\(reset)")
        print()
    }
    ...
    private func candidateInfoPlistURLs() -> [URL] {
        ...
        while true {
            ...
            // when running a standalone remux-cli binary from build/Debug.
    ...
    private func usage() -> String {
        ...
        Environment:
          ...
          REMUX_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to /tmp/remux.sock and auto-discovers tagged/debug sockets.
        """
    }
}
